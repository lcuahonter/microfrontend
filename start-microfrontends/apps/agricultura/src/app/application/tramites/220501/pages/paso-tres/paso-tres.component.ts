...
import { CategoriaMensaje, ConsultaioState, DocumentoService, DocumentosFirmaQuery, DocumentosFirmaStore,Notificacion, base64ToHex, encodeToISO88591Hex} from '@libs/shared/data-access-user/src';
 ...
 */
  ...
  constructor(private router: Router,
   ...
   this.idSolicitud = this.solicitud220501Query.getValue().id_solicitud
    this.num_autorizacion = String(this.solicitud220501Query.getValue().certificadosAutorizados) 
...
obtieneFirma(firma: string): void {
  if (!this.cadenaOriginal || !this.datosFirmaReales) {
    console.error('Missing data needed to complete the signature.');
  ...
  this.documentoService
    ...
    .pipe(
      ...
      switchMap((response) => {
       ...
       return this.revisionService.firmaDatos(
          PAYLOAD,
); }) )
 ...
 */
 getCadenaOriginal(): void {
  ...
  this.revisionService
    .generaCadenaOriginal('220501', this.idSolicitud??'', PAYLOAD)
    .pipe(take(1))
    .subscribe({
      next: (res) => {
