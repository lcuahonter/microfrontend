...
import { MERCANICA_CONFIGURACION, SALDO_PENDIENTE_NOTIFICACION } from '../../../220502/constantes/constantes';
...
import { DatosDeMercancias, MercanciaTabla } from '../../models/medio-transporte.model';
...
export class MedioTransporteComponent implements OnInit, OnDestroy, OnChanges {
   ...
   */
  solicitud220502State: Solicitud220501State = {} as Solicitud220501State;
  ...
  constructor(
    ...
    public solicitud220501Store: Solicitud220501Store,
     private solicitudService: RevisionService,
  ) {
    /** Inyectar el ControlContainer principal para administrar los controles de formulario */
  }
      ...
      this.grupoFormularioPadre.addControl(
        ...
        new FormGroup({
          transporteIdMedio: new FormControl(
            this.solicitud220502State.transporteIdMedio, [Validators.required]
          ),
  ...
  enCambioDeValor(value: string | number): void {
    this.solicitud220501Store.setEsSolicitudFerros(value);
  }
  ...
  setTransporteIdMedio(event: Catalogo): void {
    this.solicitud220501Store.setTransporteIdMedio(event.clave??'');
  }
  ...
  setTotalDeGuiasAmparadas(event: Event): void {
    const VALUE = (event.target as HTMLInputElement).value;
    this.solicitud220501Store.setTotalDeGuiasAmparadas(VALUE);
  }

  /**
   * @description
   * Marca todos los controles dentro del FormGroup especificado
   * por `claveDeControl` como tocados, sucios y actualiza su validez.
   * @returns 
   */
