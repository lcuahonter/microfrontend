import {Catalogo, CategoriaMensaje, ConsultaioState, DocumentoService, DocumentosFirmaQuery,DocumentosFirmaStore,FirmaElectronicaComponent, Notificacion, base64ToHex,encodeToISO88591Hex, formatFecha, renameKey} from '@libs/shared/data-access-user/src';
 ...
 */
  ...
  ngOnInit(): void{
   ...
   this.idSolicitud = this.solicitud220502Query.getValue().id_solicitud
    this.num_autorizacion = String(this.solicitud220502Query.getValue().certificadosAutorizados) 
 ...
 */
 getCadenaOriginal(): void {
  ...
  this.solicitudPantallasService
    .generaCadenaOriginal('220502', this.idSolicitud??'', PAYLOAD)
    .pipe(take(1))
    .subscribe({
      next: (res) => {
