...
import { CodigoRespuesta, ProcesoSolicitud } from '../core/enum/core-enum';
 ...
 */
  ...
  getValorIndice(e: AccionBoton): void {
    if (e?.valor && e.valor > 0 && e.valor < 4) {
      ...
      //nos saltamos los documentos cuando es solo datos
      if (this.iniciarAtenderRequerimientoData?.tipo_requerimiento === TipoRequerimiento.DATOS && this.indice === 2) {
 ...
 */
  ...
  /**
   * Avanza el asistente al siguiente paso y sincroniza los índices locales con el índice actual del wizard.
   *
   * @remarks Actualiza `indice` y `datosPasos.indice` usando `wizardComponent.indiceActual + 1`.
   * @returns void
   */
  continuar(): void {
    this.wizardComponent.siguiente();
