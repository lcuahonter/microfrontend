...
import { API_GET_DOCUMENTOS_ADICIONALES_REQ, API_POST_AUTORIZAR_REQUERIMIENTO_GUARDAR_OBSERVACION, API_POST_AUTORIZAR_REQUERIMIENTO_MOSTRAR_FIRMA, API_POST_DOCUMENTOS_ESPECIFICOS_REQ, API_POST_FIRMAR_AUTORIZAR_REQUERIMIENTO, API_POST_GUARDAR_REQUERIMIENTO, API_POST_GUARDAR_REQUERIMIENTO_MOSTRAR_FIRMA, DocumentosEspecificosResponse } from '@libs/shared/data-access-user/src';
...
import { MostrarFirmarAutorizarRequerimientoRequest } from '../../models/autorizar-requerimiento/request/firma-mostrar-autorizar-requerimiento.request.model';
import { MostrarFirmarAutorizarRequerimientoResponse } from '../../models/autorizar-requerimiento/response/mostrar-firmar-autorizar-requerimiento-response.model';
...
export class GuardarAutorizarRequerimientoService {
 ...
 */
  postMostrarFirma(tramite: number, numFolio: string, PAYLOAD: MostrarFirmarAutorizarRequerimientoRequest):
    Observable<BaseResponse<MostrarFirmarAutorizarRequerimientoResponse>> {
    const ENDPOINT = `${this.host}${API_POST_AUTORIZAR_REQUERIMIENTO_MOSTRAR_FIRMA(tramite.toString(), numFolio)}`;
    return this.http.post<BaseResponse<MostrarFirmarAutorizarRequerimientoResponse>>(ENDPOINT, PAYLOAD);
  }

  /** 
 * genera una observacion en autorizar requerimiento
 * @param tramite - Número de trámite al que pertenece el requerimiento
 * @param numFolio - Número de folio del trámite
 * @param PAYLOAD - Datos para la firma del requerimiento
 * @returns Observable con la respuesta del servidor para mostrar la firma
*/
  postGenerarObservacion(tramite: number, numFolio: string, PAYLOAD: ObservacionRequest):
    Observable<BaseResponse<null>> {
    const ENDPOINT = `${this.host}${API_POST_AUTORIZAR_REQUERIMIENTO_GUARDAR_OBSERVACION(tramite.toString(), numFolio)}`;
   ...
   */
    postFirmarAutorizarRequerimiento(tramite: number, numFolio: string, PAYLOAD: FirmarRequerimientoRequest):
      Observable<BaseResponse<null>> {
      const ENDPOINT = `${this.host}${API_POST_FIRMAR_AUTORIZAR_REQUERIMIENTO(tramite.toString(), numFolio)}`;
      return this.http.post<BaseResponse<null>>(ENDPOINT, PAYLOAD);
    }
  ...
  postDocumentosAdicionales(
    ...
    const ENDPOINT = `${this.host}` + API_GET_DOCUMENTOS_ADICIONALES_REQ(numFolio.toString());
    ...
    return this.http.get<BaseResponse<DocumentosEspecificosResponse[]>>(ENDPOINT, { params: PARAMS });
  }
