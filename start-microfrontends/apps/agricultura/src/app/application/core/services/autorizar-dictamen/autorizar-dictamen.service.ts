...
import {
  API_POST_FIRMAR,
  API_POST_INICIAR_AUTORIZAR_DICTAMEN,
  API_POST_MOSTRAR_FIRMAR,
  API_POST_OBSERVACION_GUARDAR,
  API_POST_OFICIO_AUTORIZACION,
  API_POST_OFICIO_RECHAZADO,
  API_POST_OFICIO_RESOLUCION_AUTORIZACION,
  API_POST_OFICIO_RESOLUCION_RECHAZADO,
  NUMFOLIOTRAMITE,
  TRAMITE,
} from '../../../constantes/autorizar-dictamen/api-constants';
...
import {
  ...
  OficioAutorizacionResponse,
} from '../../models/autorizar-requerimiento/response/oficio-autorizacion-response.model';
...
export class AutorizarDictamenService {
  ...
  */
   ...
   * @returns Observable con la respuesta del oficio generado
   */
  postOficioAutorizacionOK(tramite: number, folioTramite : string): Observable<BaseResponse<OficioAutorizacionResponse>> {
    const ENDPOINT = `${this.host}${API_POST_OFICIO_RESOLUCION_AUTORIZACION.replace(TRAMITE, tramite.toString()).replace(NUMFOLIOTRAMITE, folioTramite.toString())}`;
    return this.http.get<BaseResponse<OficioAutorizacionResponse>>(ENDPOINT);
  }
  ...
  */
   ...
   * @returns Observable con la respuesta del oficio generado
   */
  postOficioRechazadoOK(tramite: number, folioTramite : string): Observable<BaseResponse<OficioAutorizacionResponse>>{
    const ENDPOINT = `${this.host}${API_POST_OFICIO_RESOLUCION_RECHAZADO.replace(TRAMITE, tramite.toString()).replace(NUMFOLIOTRAMITE, folioTramite.toString())}`;
    return this.http.get<BaseResponse<OficioAutorizacionResponse>>(ENDPOINT);
  }
