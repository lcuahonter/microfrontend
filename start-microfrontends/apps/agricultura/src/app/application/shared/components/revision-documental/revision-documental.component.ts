...
export class RevisionDocumentalComponent implements OnInit, OnDestroy {
  ...
  guardar(esSiguiente: boolean = false): void {
    ...
    this.subscription.push(
      this.hojaTrabajoService
        .guardarHojaTrabajo(this.tramite, this.dataCompleta)
        .subscribe({
          next: (data: ResponseHojaTrabajoModel) => {
            this.hojaTrabajoAgriculturaStore.setIdHojaTrabajo({
              ...
              orden_tratamiento:
                data.datos?.orden_tratamiento?.orden_tratamiento ?? 0,
              remision_muestra:
  ...
  catalogoNombresLaboratorio(tipoLaboratorio: Catalogo): void {
    this.subscription.push(
      this.hojaTrabajoService
        .catalogoNombresLaboratorio(this.tramite, tipoLaboratorio.clave ?? '')
        .subscribe({
   ...
   */
  siguiente(): void {
    if (this.validaciones.hojaTrabajo) {
      this.guardar(true);
    }
  }
  ...
  catalogoTiposLaboratorio(): void {
    this.subscription.push(
      this.hojaTrabajoService.catalogoTiposLaboratorio(this.tramite).subscribe({
        next: (data: ResponseCatalogo) => {
  ...
  catalogoUnidadMedidaComercial(): void {
    this.subscription.push(
      this.hojaTrabajoService
        .catalogoUnidadMedidaComercial(this.tramite)
        .subscribe({
          next: (data: ResponseCatalogo) =>
