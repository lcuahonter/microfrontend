...
import { API_POST_AUTORIZAR_MOSTRAR_FIRMAR, API_POST_AUTORIZAR_OBSERVACION_REQUERIMIENTO, API_POST_DOCUMENTOS_ESPECIFICOS_REQ, API_POST_GUARDAR_REQUERIMIENTO, API_POST_GUARDAR_REQUERIMIENTO_MOSTRAR_FIRMA, DocumentosEspecificosResponse } from '@libs/shared/data-access-user/src';
...
export class GuardarRequerimientoService {
  ...
  * @returns Observable con la respuesta del servidor que incluye el ID del requerimiento guardado
  */
  postObservacionRequerimiento(tramite: number, numFolio: string, PAYLOAD: ObservacionRequest):
    Observable<BaseResponse<GuardarRequerimientosResponse>> {
    const ENDPOINT = `${this.host}${API_POST_GUARDAR_REQUERIMIENTO(tramite.toString(), numFolio)}`;
