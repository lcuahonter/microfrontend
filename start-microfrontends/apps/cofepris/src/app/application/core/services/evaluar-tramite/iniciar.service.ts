import { API_POST_INICIAR_AUTORIZAR_REQUERIMIENTO, API_POST_INICIAR_VERIFICAR_REQUERIMIENTO, ENVIRONMENT } from '@libs/shared/data-access-user/src';
...
import { GenerarDictamenResponse } from '../../models/evaluar/response/evaluar-estado-evaluacion-response.model';
...
export class IniciarService {
  ...
  postIniciarDictamen(tramite: number, numFolio: string, PAYLOAD: IniciarRequerimientoRequest):
    Observable<BaseResponse<GenerarDictamenResponse>> {
    const ENDPOINT = `${this.host}${API_GET_INICAR_DICTAMEN(tramite.toString(), numFolio)}`;

    return this.http.post<BaseResponse<GenerarDictamenResponse>>(ENDPOINT, PAYLOAD);
  }
   ...
   * @returns Observable con la respuesta del servidor del requerimiento iniciado
   */
  postVerificarIniCiarRequerimiento(tramite: number, numFolio: string, PAYLOAD: IniciarRequerimientoRequest):
    Observable<BaseResponse<IniciarRequerimientoResponse>> {
    const ENDPOINT = `${this.host}${API_POST_INICIAR_VERIFICAR_REQUERIMIENTO(tramite.toString(), numFolio)}`;
    return this.http.post<BaseResponse<IniciarRequerimientoResponse>>(ENDPOINT, PAYLOAD);
  }
   ...
   * @returns Observable con la respuesta del servidor del requerimiento iniciado
   */
  postAutorizarIniCiarRequerimiento(tramite: number, numFolio: string, PAYLOAD: IniciarRequerimientoRequest):
    Observable<BaseResponse<IniciarRequerimientoResponse>> {
    const ENDPOINT = `${this.host}${API_POST_INICIAR_AUTORIZAR_REQUERIMIENTO(tramite.toString(), numFolio)}`;
    return this.http.post<BaseResponse<IniciarRequerimientoResponse>>(ENDPOINT, PAYLOAD);
  }
