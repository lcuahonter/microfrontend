...
import { API_POST_FIRMAR, API_POST_INICIAR_AUTORIZAR_DICTAMEN, API_POST_MOSTRAR_FIRMAR, API_POST_OBSERVACION_GUARDAR, API_POST_OFICIO_AUTORIZACION, API_POST_OFICIO_PARCIAL, API_POST_OFICIO_RECHAZADO, NUMFOLIOTRAMITE, TRAMITE} from '../../../constantes/autorizar-dictamen/api-constants';
...
import { IniciarAutorizacionResponse } from '../../models/autorizar-dictamen/iniciar-autorizar-dictamen-response.model';
...
export class AutorizarDictamenService {
  ...
  */
   ...
   * @returns Observable con la respuesta del oficio generado
  */
  postOficioParcial(tramite: number, idSolicitud : number, PAYLOAD?: Firma | null): Observable<BaseResponse<DocumentoOficialResponse>>{
    const ENDPOINT = `${this.host}${API_POST_OFICIO_PARCIAL.replace(TRAMITE, tramite.toString()).replace(IDSOLICITUD, idSolicitud.toString())}`;
    const BODY = PAYLOAD ? PAYLOAD : null;
    return this.http.post<BaseResponse<DocumentoOficialResponse>>(ENDPOINT, BODY);
  }
