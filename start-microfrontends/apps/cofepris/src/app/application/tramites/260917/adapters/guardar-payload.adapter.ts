...
import {Solicitud260917State} from '../estados/tramites/tramite260917.store';
import {Tramite260917Store} from '../estados/tramites/tramite260917.store';
import {TablaScianConfig,TablaMercanciasDatos} from '../../../shared/models/datos-solicitud.model';
@Injectable({
  providedIn: 'root'
})
export class GuardarAdapter_260917 {
  static toFormPayload( state: Solicitud260917State ,DatosDelSolicitudDatos:DatosDelSolicituteSeccionState ,manifestoDatos:DatosDomicilioLegalState,representanteLegalDatos:DomicilioState,terceros_derechosDatos:Solicitud260917State): unknown {
    ...
    return {
        ...
        "solicitud": {
            ...
            "aduanaAIFA": DatosDelSolicitudDatos.aifaCheckbox,
              ...
              "tipoOperacion": DatosDelSolicitudDatos.ideGenerica === '1' ? 'MO' : DatosDelSolicitudDatos.ideGenerica === '2' ? 'MP' : DatosDelSolicitudDatos.ideGenerica === '0' ? 'PR' : '',
            "justificacion": DatosDelSolicitudDatos.observaciones
    ...
    /**
      ...
      static fromApiResponse(response: unknown): Partial<Solicitud260917State> {
          ...
          const PARTIAL: Partial<Solicitud260917State> = {
   ...
   /**
       ...
       * @returns Partial<Tramite260917State> (and patches the store when provided)
       */
      static patchToStore(response: any, store?: Tramite260917Store): Partial<Solicitud260917State> {
          const PARTIAL = GuardarAdapter_260917.fromApiResponse(response);
          if (store) {
    ...
    private static mapEstablecimientoDatos(establecimiento: any): Partial<DatosDelSolicituteSeccionState> {
      ...
      static patchToStoreDatosSolicitud(response: any, store?: DatosDelSolicituteSeccionStateStore): Partial<DatosDelSolicituteSeccionState> {
          const PARTIAL = GuardarAdapter_260917.fromApiResponseDatosSolicitud(response);
          if (store) {
              store.update((state) => ({ ...state, ...PARTIAL }));
          }
          return PARTIAL;
    ...
    private static mapMercanciasData(arr?: any[]): any[] {
        ...
        return (arr ?? []).map((m: any) => ({
           ...
           cantidadUMC: m?.unidadMedidaComercial?.descripcion || 'test',
    ...
    static patchToStoreDomicilio(response: any, store?: DomicilioStore): Partial<DomicilioState> {
        const PARTIAL = GuardarAdapter_260917.fromApiResponseRepresentanteLegal(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }   
        return PARTIAL;
    ...
    static patchToStoreManifestos(response: any, store?: DatosDomicilioLegalStore): Partial<DatosDomicilioLegalState> {
        const PARTIAL = GuardarAdapter_260917.fromApiResponseManifesto(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
