...
export class GuardarAdapter_260215 {
  /**
  ...
  static toFormPayload(state: Solicitud260215State): unknown {
    return {
        ...
        "solicitud": {
            ...
            "declaracionesSeleccionadas":state.datosSolicitudFormState.manifesto ?? false,
        ...
        "datosSCIAN": Array.isArray(state.scianConfigDatos) && state.scianConfigDatos.length > 0 ? state.scianConfigDatos.map((datos) => ({
            "cveScian": datos.clave,
            "descripcion": datos.descripcion,
            "selected": true
        })) : [],
        "mercancias": Array.isArray(state.tablaMercanciasConfigDatos) && state.tablaMercanciasConfigDatos.length > 0 ? state.tablaMercanciasConfigDatos.map((mercancia) => ({
            ...
            "nombreCortoPaisOrigen": Array.isArray(mercancia.paisOrigenDatosClave) ? mercancia.paisOrigenDatosClave : [],
            "nombreCortoPaisProcedencia": Array.isArray(mercancia.paisProcedenciaDatosClave) ? mercancia.paisProcedenciaDatosClave : [],
            "idTipoProductoTipoTramite": mercancia.tipoProducto,
            "tipoProductoDescripcionOtros": mercancia.especifique,
            "nombreCortoUsoEspecifico": Array.isArray(mercancia.usoEspecificoDatosClave) ? mercancia.usoEspecificoDatosClave : [],
        ...
        "gridTerceros_TIPERS_FAB": Array.isArray(state.fabricanteTablaDatos) && state.fabricanteTablaDatos.length > 0 ? state.fabricanteTablaDatos.map((fabricante) => ({
            "idPersonaSolicitud": "",
            "ideTipoTercero": "",
        ...
        "gridTerceros_TIPERS_DES": Array.isArray(state.destinatarioFinalTablaDatos) && state.destinatarioFinalTablaDatos.length > 0 ? state.destinatarioFinalTablaDatos.map((destinatario) => ({
            "idPersonaSolicitud": "",
        ...
        "gridTerceros_TIPERS_PVD": Array.isArray(state.proveedorTablaDatos) && state.proveedorTablaDatos.length > 5 ? state.proveedorTablaDatos.map((proveedor) => ({
            "idPersonaSolicitud": "",
            "ideTipoTercero": "",
            ...
            "domicilio": {
                ...
                "colonia":  {
                    "clave": proveedor.coloniaObj?.clave||"",
                    "nombre": proveedor.coloniaObj?.descripcion || ""
                },
                "delegacionMunicipio": 
                 {
                    "clave": proveedor.delegacionMunicipioObj?.clave||"",
                    "nombre": proveedor.delegacionMunicipioObj?.descripcion || ""
                },
                "localidad": {
                    "clave": proveedor.localidadObj?.clave||"",
                    "nombre": proveedor.localidadObj?.descripcion || ""
                },
                "entidadFederativa":{
                    "clave": proveedor.entidadFederativaObj?.clave||"",
                    "nombre": proveedor.entidadFederativaObj?.descripcion || ""
                },
                "informacionExtra": "",
        ...
        "gridTerceros_TIPERS_FAC": Array.isArray(state.facturadorTablaDatos) && state.facturadorTablaDatos.length > 5 ? state.facturadorTablaDatos.map((facturador) => ({
            "idPersonaSolicitud": "",
            "ideTipoTercero": "TIPERS.FAB",
            "personaMoral": facturador.tipoPersona === "Moral" ? "1" : "0",
            "booleanExtranjero": facturador.nacionalidad === 'Extranjero' ? "1" : "0",
            "booleanFisicaNoContribuyente": "0",
            "denominacion": facturador.razonSocial ?? facturador.nombreRazonSocial ?? "",
            "razonSocial": facturador.razonSocial ?? facturador.nombreRazonSocial ?? "",
            "rfc": facturador.rfc ?? "",
            ...
            "domicilio": {
                ...
                "colonia": {
                    "clave": facturador.coloniaObj?.clave ?? "",
                    "nombre": facturador.coloniaObj?.descripcion ?? ""
                } ,
                "delegacionMunicipio":{
                    "clave": facturador.municipioAlcaldiaObj?.clave ?? "",
                    "nombre": facturador.municipioAlcaldiaObj?.descripcion ?? ""
                },
                "localidad":{
                    "clave": facturador.localidadObj?.clave ?? "",
                    "nombre": facturador.localidadObj?.descripcion ?? ""
                },
                "entidadFederativa": {
                    "clave": facturador.entidadFederativaObj?.clave ?? "",
                    "nombre": facturador.entidadFederativaObj?.descripcion ?? ""
                },
                "informacionExtra": "",
   ...
   private static mapScianData(arr?: any[]): any[] {
    ...
    /**
     * Map an API response (form payload) back into a partial Solicitud260215State.
     ...
     * @returns Partial<Solicitud260215State>
     */
    // eslint-disable-next-line complexity
    static fromApiResponse(response: unknown): Partial<Solicitud260215State> {
        ...
        const DATOS_SOLICITUD_FORM_STATE = {
            ...
            aeropuerto: false,
            manifesto: RESP.solicitud?.declaracionesSeleccionadas ?? false,
        };

        const PARTIAL: Partial<Solicitud260215State> = {
     ...
     * @param store Optional Tramite260215Store instance to apply the patch
     * @returns Partial<Solicitud260215State> (and patches the store when provided)
     */
    static patchToStore(response: any, store?: Tramite260215Store): Partial<Solicitud260215State> {
        const PARTIAL = GuardarAdapter_260215.fromApiResponse(response);
        if (store) {
