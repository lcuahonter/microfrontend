...
export class GuardarAdapter_260512 {
  ...
  : [],
     ...
     idSolicitud: GuardarAdapter_260512.idSolicitud
    };
  }
       ...
       * @param store Optional Tramite260512Store instance to apply the patch
       ...
       */
       ...
       static patchToStoreEstable(response: any, store?: AvisocalidadStore): void {
              const PARTIAL = GuardarAdapter_260512.fromApiResponseToEstablecimientoDatos(response);
              if (store) {
       ...
       static patchToStoreSolicitud(response: any, store?: DatosDomicilioLegalStore): void {
              const PARTIAL = GuardarAdapter_260512.fromApiResponseToSolicitudDatos(response);
              if (store) {
                  // Akita's update accepts a partial or updater function
                  store.update((state) => ({ ...state, ...PARTIAL }));
              }
             ...
             static patchToStorePago(response: any, store?: PagoDerechosStore): void {
              const PARTIAL = GuardarAdapter_260512.fromApiResponseToPago(response);
              if (store) {
                  // Akita's update accepts a partial or updater function
                  store.update((state) => ({ ...state, ...PARTIAL }));
              }
       ...
       static patchresponseToStore(response: any, avisocalidadStore: AvisocalidadStore, solicitudStore: DatosDomicilioLegalStore, pagoStore: PagoDerechosStore): void {
              GuardarAdapter_260512.patchToStoreEstable(response, avisocalidadStore);
              GuardarAdapter_260512.patchToStoreSolicitud(response, solicitudStore);
              GuardarAdapter_260512.patchToStorePago(response, pagoStore);
              GuardarAdapter_260512.idSolicitud=Number((response as any).idSolicitud);
          }
