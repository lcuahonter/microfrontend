  ...
  */
  public alHacerClicEnElBoton(event: ModeloDeFormaDinamica): void {
    if (event.campo === 'buscar') {
      this.cancelacionPeticionService.getBuscarDatos(this.ninoFormGroup.get('rfc')?.value || '').pipe(
      takeUntil(this.destruirNotificador$),
    map((response) => {  
  ...
  /**
   * Marca todos los controles del formulario como "tocados" para activar la validación y mostrar mensajes de error.
   * 
   * Utiliza el método `markAllAsTouched()` sobre el formulario para asegurar que todos los campos sean validados,
   * incluso si el usuario no ha interactuado con ellos.
   */
