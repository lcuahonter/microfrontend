...
export class GuardarAdapter_260207 {
  ...
  return {
   ...
   private static mapScianData(arr?: any[]): any[] {
    ...
    /**
     * Map an API response (form payload) back into a partial Tramite260207State.
     ...
     * @returns Partial<Tramite260207State>
     */
    // eslint-disable-next-line complexity
    static fromApiResponse(response: unknown): Partial<Tramite260207State> {
        ...
        const PARTIAL: Partial<Tramite260207State> = {
     ...
     * @param store Optional Tramite260207Store instance to apply the patch
     * @returns Partial<Tramite260207State> (and patches the store when provided)
     */
    static patchToStore(response: any, store?: Tramite260207Store): Partial<Tramite260207State> {
        const PARTIAL = GuardarAdapter_260207.fromApiResponse(response);
        if (store) {
