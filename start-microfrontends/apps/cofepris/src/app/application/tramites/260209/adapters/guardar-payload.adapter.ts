 ...
 * para el trámite de tratamientos especiales 260209.
 */
...
export class GuardarAdapter_260209 {
  ...
  return {
   ...
   private static mapScianData(arr?: any[]): any[] {
    ...
    /**
     * Map an API response (form payload) back into a partial Tramite260209State.
     ...
     * @returns Partial<Tramite260209State>
     */
    // eslint-disable-next-line complexity
    static fromApiResponse(response: unknown): Partial<Tramite260209State> {
        ...
        const PARTIAL: Partial<Tramite260209State> = {
     ...
     * @param store Optional Tramite260209Store instance to apply the patch
     * @returns Partial<Tramite260209State> (and patches the store when provided)
     */
    static patchToStore(response: any, store?: Tramite260209Store): Partial<Tramite260209State> {
        const PARTIAL = GuardarAdapter_260209.fromApiResponse(response);
        if (store) {
