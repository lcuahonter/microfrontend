/**
 * @fileoverview Este archivo define el estado y las operaciones relacionadas con el trámite 260918.
...
/**
 * @interface Tramite260918State
 * @description Define la estructura del estado para el trámite 260918.
 */
export interface Tramite260918State {
  idSolicitud: number;
 ...
 * @description Crea el estado inicial para el trámite 260918.
 * @returns {Tramite260918State} Estado inicial con valores predeterminados.
 */
export function createInitialState(): Tramite260918State {
  return {
    idSolicitud: 0,
    btonDeRadio: '',
    justificacion: '',
...
/**
 * @class Tramite260918Store
 * @description Clase que extiende la funcionalidad de Akita Store para gestionar el estado
 * del trámite 260918. Proporciona métodos para actualizar propiedades específicas del estado.
 */
@Injectable({
  providedIn: 'root',
})
@StoreConfig({ name: 'tramite260918', resettable: true })
export class Tramite260918Store extends Store<Tramite260918State> {
  /**
   * @constructor
   ...
   */
  setTramite260918State(valores: Partial<Tramite260918State>): void {
    this.update((state => ({
      ...state,
      ...valores,
    })));
