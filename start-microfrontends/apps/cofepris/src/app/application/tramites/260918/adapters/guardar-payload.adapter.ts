...
import {Tramite260918State} from '../estados/tramite260918.store'
import {Tramite260918Store} from '../estados/tramite260918.store'

@Injectable({
  providedIn: 'root'
})
export class GuardarAdapter_260918 {
  static toFormPayload( state: Tramite260918State ,DatosDelSolicitudDatos:DatosDelSolicituteSeccionState ,manifestoDatos:DatosDomicilioLegalState,representanteLegalDatos:DomicilioState,terceros_derechosDatos:Tramite260918State): unknown {
    ...
    /**
      ...
      static fromApiResponse(response: unknown): Partial<Tramite260918State> {
          ...
          const PARTIAL: Partial<Tramite260918State> = {
   ...
   /**
       ...
       * @returns Partial<Tramite260918State> (and patches the store when provided)
       */
      static patchToStore(response: any, store?: Tramite260918Store): Partial<Tramite260918State> {
          const PARTIAL = GuardarAdapter_260918.fromApiResponse(response);
          if (store) {
    ...
    private static mapEstablecimientoDatos(establecimiento: any): Partial<DatosDelSolicituteSeccionState> {
      ...
      static patchToStoreDatosSolicitud(response: any, store?: DatosDelSolicituteSeccionStateStore): Partial<DatosDelSolicituteSeccionState> {
          const PARTIAL = GuardarAdapter_260918.fromApiResponseDatosSolicitud(response);
          if (store) {
              store.update((state) => ({ ...state, ...PARTIAL }));
          }
          return PARTIAL;
    ...
    static patchToStoreDomicilio(response: any, store?: DomicilioStore): Partial<DomicilioState> {
        const PARTIAL = GuardarAdapter_260918.fromApiResponseRepresentanteLegal(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }   
        return PARTIAL;
    ...
    static patchToStoreManifestos(response: any, store?: DatosDomicilioLegalStore): Partial<DatosDomicilioLegalState> {
        const PARTIAL = GuardarAdapter_260918.fromApiResponseManifesto(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
