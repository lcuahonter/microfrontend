...
export class GuardarAdapter_260205 {
  ...
  private static mapScianData(arr?: any[]): any[] {
    ...
    /**
     * Map an API response (form payload) back into a partial Tramite260205State.
     ...
     * @returns Partial<Tramite260205State>
     */
    // eslint-disable-next-line complexity
    static fromApiResponse(response: unknown): Partial<Tramite260205State> {
        ...
        const PARTIAL: Partial<Tramite260205State> = {
     ...
     * @param store Optional Tramite260205Store instance to apply the patch
     * @returns Partial<Tramite260205State> (and patches the store when provided)
     */
    static patchToStore(response: any, store?: Tramite260205Store): Partial<Tramite260205State> {
        const PARTIAL = GuardarAdapter_260205.fromApiResponse(response);
        if (store) {
