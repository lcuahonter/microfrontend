...
export class GuardarAdapter_260912 {
    /**
     ...
     */
    static toFormPayload(state: Tramites260912State, datosState: DatosDelSolicituteSeccionState, manifestoState: DatosDomicilioLegalState, representanteLegalDatos: DomicilioState): unknown {
        return {
            "solicitante": {
                "rfc": "AAL0409235E6",
            ...
            "establecimiento": {
                ...
                "avisoFuncionamiento": datosState.avisoCheckbox.toString(),
     ...
     */
    // eslint-disable-next-line complexity
    static fromApiResponse(response: unknown): Partial<Tramites260912State> {
        ...
        const PARTIAL: Partial<Tramites260912State> = {
     ...
     * @param store Optional Tramite260912Store instance to apply the patch
     * @returns Partial<Tramites260912State> (and patches the store when provided)
     */
    static patchToStore(response: any, store?: Tramite260912Store): Partial<Tramites260912State> {
        const PARTIAL = GuardarAdapter_260912.fromApiResponse(response);
        if (store) {
    ...
    static patchToStoreDatosSolicitud(response: any, store?: DatosDelSolicituteSeccionStateStore): Partial<DatosDelSolicituteSeccionState> {
        const PARTIAL = GuardarAdapter_260912.fromApiResponseDatosSolicitud(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
    ...
    static patchToStoreDomicilio(response: any, store?: DomicilioStore): Partial<DomicilioState> {
        const PARTIAL = GuardarAdapter_260912.fromApiResponseRepresentanteLegal(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }   
        return PARTIAL;
    ...
    static patchToStoreManifestos(response: any, store?: DatosDomicilioLegalStore): Partial<DatosDomicilioLegalState> {
        const PARTIAL = GuardarAdapter_260912.fromApiResponseManifesto(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
