...
import {
  Tramite260104State,
  Tramite260104Store,
} from '../../estados/tramite260104Store.store';
 ...
 * @description
 * Componente que actúa como contenedor para manejar los datos de la solicitud del trámite 260104.
...
export class ContenedorDeDatosSolicitudComponent implements OnInit, OnDestroy {
  ...
  /**
   * @property {Tramite260104State} tramiteState
   * @description
   * Estado actual del trámite 260104.
   */
  public tramiteState!: Tramite260104State;
   ...
   *
   * @param {Tramite260104Query} tramite260104Query - Consulta para acceder al estado del trámite.
   * @param {Tramite260104Store} tramite260104Store - Tienda para actualizar el estado del trámite.
   */
  constructor(
   ...
   */
  ngOnInit(): void {
    this.tramite260104Query.selectTramiteState$
      .pipe(
      ...
      .subscribe((seccionState) => {
        if(!seccionState.create && seccionState.procedureId === '260104') {
   ...
   *
   * Actualiza la configuración de datos en el store `tramite260104Store`
   * con las opciones seleccionadas.
   */
  opcionSeleccionado(event: TablaOpcionConfig[]): void {
    this.tramite260104Store.updateOpcionConfigDatos(event);
  }
   ...
   *
   * Este método actualiza los datos de configuración SCIAN en el estado del trámite 260104
   * utilizando el evento proporcionado.
   */
  scianSeleccionado(event: TablaScianConfig[]): void {
    this.tramite260104Store.updateScianConfigDatos(event);
  }
  ...
  mercanciasSeleccionado(event: TablaMercanciasImportacion[]): void {
    this.tramite260104Store.updateTablaMercanciasConfigDatos(event);
  }
  ...
  datasolicituActualizar(event: DatosSolicitudFormState): void {
    this.tramite260104Store.updateDatosSolicitudFormState(event);
  }
  ...
  datosDeTablaSeleccionados(event: DatosDeTablaSeleccionados): void {
    this.tramite260104Store.update((state) => ({
      ...state,
  ...
  claveSeleccionada(event: TablaMercanciaClaveConfig[]): void {
    this.tramite260104Store.updateTablaMercanciaClaveConfigDatos(event);
  }
  /**
   * @method ngOnDestroy
   * @description
