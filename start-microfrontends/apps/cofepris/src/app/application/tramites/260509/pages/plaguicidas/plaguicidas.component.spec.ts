...
describe('PlaguicidasComponent', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      ...
      providers: [
        { provide: Tramite260509Store, useClass: MockStore },
        { provide: Tramite260509Query, useClass: MockQuery },
...
it('should call wizardComponent.siguiente() when navigating forward (indice > 1)', () => {
  ...
  it('should handle cerrarModal(true) and proceedNavigation', () => {
    const proceedSpy = jest.spyOn(component as any, 'proceedNavigation').mockImplementation(() => {});
    component.isPeligro = true;
    (component as any).isPaymentRequired = true;
    ...
    expect((component as any).isPaymentRequired).toBe(false);
    expect(proceedSpy).toHaveBeenCalled();
    proceedSpy.mockRestore();
  });

  it('should handle cerrarModal(false)', () => {
    (component as any).isPaymentRequired = false;
    component.mostrarAlerta = true;
    component.cerrarModal(false);
    expect(component.mostrarAlerta).toBe(false);
    expect((component as any).isPaymentRequired).toBe(true);
  });
