...
export class GuardarAdapter_260903 {
    /**
     ...
     */
    static toFormPayload(state: Tramite260903State, datosState: DatosDelSolicituteSeccionState, manifestoState: DatosDomicilioLegalState, representanteLegalDatos: DomicilioState): unknown {
        return {
            "idSolicitud": "0",
            "solicitante": {
                "rfc": "AAL0409235E6",
    ...
    private static mapMercanciasData(arr?: any[]): any[] {
        ...
        return (arr ?? []).map((m: any) => ({
            ...
            clasificacion: m.idClasificacionProducto ?? '',
            ...
            cantidadUMC: GuardarAdapter_260903.obtenerParteEntera(m?.cantidadUMCConComas) || '',
            ...
            cantidadUMT: GuardarAdapter_260903.obtenerParteEntera(m.cantidadUMTConComas) ?? '',
     ...
     */
    // eslint-disable-next-line complexity
    static fromApiResponse(response: unknown): Partial<Tramite260903State> {
        ...
        const PARTIAL: Partial<Tramite260903State> = {
     ...
     */
    static patchToStore(response: any, store?: Tramite260903Store): Partial<Tramite260903State> {
        const PARTIAL = GuardarAdapter_260903.fromApiResponse(response);
        if (store) {
    ...
    static patchToStoreDatosSolicitud(response: any, store?: DatosDelSolicituteSeccionStateStore): Partial<DatosDelSolicituteSeccionState> {
        const PARTIAL = GuardarAdapter_260903.fromApiResponseDatosSolicitud(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
    ...
    static patchToStoreDomicilio(response: any, store?: DomicilioStore): Partial<DomicilioState> {
        const PARTIAL = GuardarAdapter_260903.fromApiResponseRepresentanteLegal(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
    ...
    static patchToStoreManifestos(response: any, store?: DatosDomicilioLegalStore): Partial<DatosDomicilioLegalState> {
        const PARTIAL = GuardarAdapter_260903.fromApiResponseManifesto(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
