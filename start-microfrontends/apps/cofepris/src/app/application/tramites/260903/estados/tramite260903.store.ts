/**
 * @fileoverview Este archivo define el estado y las operaciones relacionadas con el trámite 260903.
...
/**
 * @interface Tramite260903State
 * @description Define la estructura del estado para el trámite 260903.
 */
export interface Tramite260903State {
  idSolicitud: number;
 ...
 * @description Crea el estado inicial para el trámite 260903.
 * @returns {Tramite260903State} Estado inicial con valores predeterminados.
 */
export function createInitialState(): Tramite260903State {
  return {
    idSolicitud: 0,
    btonDeRadio: '',
    justificacion: '',
...
/**
 * @class Tramite260903Store
 * @description Clase que extiende la funcionalidad de Akita Store para gestionar el estado
 * del trámite 260903. Proporciona métodos para actualizar propiedades específicas del estado.
 */
@Injectable({
  providedIn: 'root',
})
@StoreConfig({ name: 'tramite260903', resettable: true })
export class Tramite260903Store extends Store<Tramite260903State> {
  /**
   * @constructor
   ...
   */
  setTramite260903State(valores: Partial<Tramite260903State>): void {
    this.update((state => ({
      ...state,
      ...valores,
    })));
