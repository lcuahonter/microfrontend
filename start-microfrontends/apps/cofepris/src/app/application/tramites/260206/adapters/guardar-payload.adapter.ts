...
export class GuardarAdapter_260206 {
  ...
  static toFormPayload(state: Tramite260206State): unknown {
   ...
   private static mapScianData(arr?: any[]): any[] {
      ...
      /**
       * Map an API response (form payload) back into a partial Tramite260206State.
       ...
       * @returns Partial<Tramite260206State>
       */
      // eslint-disable-next-line complexity
      static fromApiResponse(response: unknown): Partial<Tramite260206State> {
          ...
          const PARTIAL: Partial<Tramite260206State> = {
       ...
       * @param store Optional Tramite260206Store instance to apply the patch
       * @returns Partial<Tramite260206State> (and patches the store when provided)
       */
      static patchToStore(response: any, store?: Tramite260206Store): Partial<Tramite260206State> {
          const PARTIAL = GuardarAdapter_260206.fromApiResponse(response);
          if (store) {
