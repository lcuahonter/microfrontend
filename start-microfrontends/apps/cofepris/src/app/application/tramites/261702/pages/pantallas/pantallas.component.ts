 ...
 */
   ...
   * @param tramite261702Query Consulta de estado para el trámite 261701.
      ...
      .pipe(
        ...
        map((seccionState) => {
          ...
          if (
            ...
            this.idSolicitud = this.cancelacionPeticion261702State['idSolicitud'] as number;
          }
        })
      )
      .subscribe();
 ...
 */
 ...
 this.esFormaValido = this.verificarLaValidezDelFormulario();
  ...
  private shouldNavigate$(): Observable<boolean> {
    return this.retirosCofeprisService.getAllState().pipe(
      take(1),
   ...
   */
  guardar(data:RetirosCofepris261702State): Promise<JSONResponse> {
    const PAYLOAD = this.ampliacionServiciosAdapter.toFormGuardarPayload(data);
    return new Promise((resolve, reject) => {
      this.retirosCofeprisService.guardarDatosPost(PAYLOAD).subscribe(response => {
        const API_RESPONSE = doDeepCopy(response);
        this.tramite261702Store.setDynamicFieldValue('idSolicitud', API_RESPONSE.datos?.id_solicitud);
