...
export class GuardarAdapter_260203 {
  ...
  private static mapScianData(arr?: any[]): any[] {
     ...
     */
    // eslint-disable-next-line complexity
    static fromApiResponse(response: unknown): Partial<Tramite260203State> {
        ...
        const PARTIAL: Partial<Tramite260203State> = {
     ...
     * @param store Optional Tramite260203Store instance to apply the patch
     * @returns Partial<Tramite260203State> (and patches the store when provided)
     */
    static patchToStore(response: any, store?: Tramite260203Store): Partial<Tramite260203State> {
        const PARTIAL = GuardarAdapter_260203.fromApiResponse(response);
        if (store) {
