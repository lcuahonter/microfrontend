...
export class GuardarAdapter_260516 {
  ...
  constructor(private establecimientQuery:AvisocalidadQuery, 
    ...
    : [],
       ...
       idSolicitud: GuardarAdapter_260516.idSolicitud
      };
    }
         ...
         * @param store Optional Tramite260516Store instance to apply the patch
         ...
         */
         ...
         static patchToStoreEstable(response: any, store?: AvisocalidadStore): void {
                const PARTIAL = GuardarAdapter_260516.fromApiResponseToEstablecimientoDatos(response);
                if (store) {
         ...
         static patchToStoreSolicitud(response: any, store?: DatosDomicilioLegalStore): void {
                const PARTIAL = GuardarAdapter_260516.fromApiResponseToSolicitudDatos(response);
                if (store) {
                    // Akita's update accepts a partial or updater function
                    store.update((state) => ({ ...state, ...PARTIAL }));
                }
               ...
               static patchToStorePago(response: any, store?: PagoDerechosStore): void {
                const PARTIAL = GuardarAdapter_260516.fromApiResponseToPago(response);
                if (store) {
                    // Akita's update accepts a partial or updater function
                    store.update((state) => ({ ...state, ...PARTIAL }));
                }
         ...
         static patchresponseToStore(response: any, avisocalidadStore: AvisocalidadStore, solicitudStore: DatosDomicilioLegalStore, pagoStore: PagoDerechosStore): void {
                GuardarAdapter_260516.patchToStoreEstable(response, avisocalidadStore);
                GuardarAdapter_260516.patchToStoreSolicitud(response, solicitudStore);
                GuardarAdapter_260516.patchToStorePago(response, pagoStore);
                GuardarAdapter_260516.idSolicitud=Number((response as any).idSolicitud);
            }
  }