...
import { Solicitud260303State, Tramite260303Store } from '../Estados/tramite260303.store';
import { EstadoCatalogResponse } from '../models/certificados-licencias-permisos.model';
import { HttpClient } from '@angular/common/http';
import { Injectable } from '@angular/core';
import { PROC_260303 } from '../../260303/servers/api-route';
...
export class CertificadosLicenciasPermisosService {
  ...
  constructor(private http: HttpClient, private tramite260303Store: Tramite260303Store,
    public query: Tramite260303Query, public tramite2603Query: Tramite2603Query
  ) {
    //
  }
 ...
 */
   ...
   * @param body - Objeto que contiene los datos del formulario a guardar.
   * @returns Un `Observable` de tipo `JSONResponse` que contiene la respuesta del servidor.
   */
  guardarDatosPost(body: Record<string, unknown>): Observable<JSONResponse> {
    return this.http.post<JSONResponse>(PROC_260303.GUARDAR, body);
  }
  /**
   * Obtiene todo el estado actual del trámite 2603 desde el store.
   * @returns Un `Observable` de tipo `Solicitud2603State` que contiene todo el estado del trámite 2603.
   */
  getAllState(): Observable<Solicitud2603State> {
    return this.tramite2603Query.selectSolicitud$;
  }
}
