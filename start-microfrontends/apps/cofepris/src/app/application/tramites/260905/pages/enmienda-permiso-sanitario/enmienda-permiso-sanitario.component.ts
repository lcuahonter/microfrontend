...
import { Datos260905Component } from '../datos-260905/datos-260905.component';
...
export class EnmiendaPermisoSanitarioComponent implements OnInit, OnDestroy {
  ...
  */
  @ViewChild(Datos260905Component) pasoUnoComponent!: Datos260905Component;
 ...
 */
  ...
  constructor(
    private tramite260905Query: Tramite260905Query,
    private tramite260905Store: Tramite260905Store, 
  ...
  ngOnInit(): void {
    this.tramite260905Query.selectTramiteState$.pipe(
      takeUntil(this.destroyNotifier$))
    .subscribe((data) => {
      this.storeData = data;
    });
  ...
  getValorIndice(e: AccionBoton): void {
    if (e.accion === 'cont') {
      ...
      const PAYLOAD = GuardarAdapter_260905.toFormPayload(this.storeData, this.datosDelSolicitud, this.manifestoState, this.domicilioState );
      let shouldNavigate = false;
      this.registroSolicitudService.postGuardarDatos('260905', PAYLOAD).subscribe(response => {
        ...
        if(shouldNavigate) {
          if(esValidObject(response) && esValidObject(response.datos)) {
            ...
            this.tramite260905Store.setIdSolicitud(ID_SOLICITUD);
          }
