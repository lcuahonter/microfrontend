...
import { Tramite260905State, Tramite260905Store } from '../estados/tramite260905Store.store';
...
export class GuardarAdapter_260905 {
    /**
     ...
     */
    static toFormPayload(state: Tramite260905State, datosState: DatosDelSolicituteSeccionState, manifestoState: DatosDomicilioLegalState, representanteLegalDatos: DomicilioState): unknown {
        return {
            "idSolicitud": "0",
            "solicitante": {
                "rfc": "AAL0409235E6",
            ...
            "solicitud": {
                ...
                "tipoOperacion": datosState.ideGenerica === '1' ? 'TIMOD.MOD' : datosState.ideGenerica === '2' ? 'TIMOD.MYP' : datosState.ideGenerica === '0' ? 'TIMOD.PRO' : '',
            ...
            "mercancias": (datosState.mercanciaDatos ?? []).map((mercancia) => {
                return {
                    ...
                    "unidadMedidaComercial": {
                        "descripcion": mercancia.unidad
                    },
                    "cantidadUMCConComas": GuardarAdapter_260905.obtenerParteEntera(mercancia?.cantidadUMC) || null,
                    "unidadMedidaTarifa": {
                        "descripcion": mercancia.unidadUMT
                    },
                    "cantidadUMTConComas": GuardarAdapter_260905.obtenerParteEntera(mercancia?.cantidadUMT) || null,
    ...
    private static mapMercanciasData(arr?: any[]): any[] {
        ...
        return (arr ?? []).map((m: any) => ({
            ...
            estadoFormaFarmaceutica: m.formaFarmaceuticaDescripcionOtros ?? '',
            formaFarmaceutica: m.idFormaFarmaceutica ?? '',
            ...
            cantidadUMC: GuardarAdapter_260905.obtenerParteEntera(m?.cantidadUMCConComas) || '',
            cantidadUmtValor: m.cantidadUMTConComas ?? '',
            unidadUMT: m?.unidadMedidaTarifa?.descripcion || '',
            unidad: m?.unidadMedidaComercial?.descripcion || '',
            cantidadUMT: GuardarAdapter_260905.obtenerParteEntera(m.cantidadUMTConComas) ?? '',
     ...
     */
    // eslint-disable-next-line complexity
    static fromApiResponse(response: unknown): Partial<Tramite260905State> {
        ...
        const PARTIAL: Partial<Tramite260905State> = {
    ...
    private static mapSolicitudDatos(solicitud: any): Partial<DatosDelSolicituteSeccionState> {
        const TIPO_OPERACION_REVERSE_MAP: Record<string, string> = {
            'TIMOD.MOD': '1',
            'TIMOD.MYP': '2',
            'TIMOD.PRO': '0'
        };
        ...
        return {
            ideGenerica: TIPO_OPERACION_REVERSE_MAP[solicitud?.tipoOperacion] ?? '',
     ...
     * @returns Partial<Tramite260905State> (and patches the store when provided)
     */
    static patchToStore(response: any, store?: Tramite260905Store): Partial<Tramite260905State> {
        const PARTIAL = GuardarAdapter_260905.fromApiResponse(response);
        if (store) {
    ...
    static patchToStoreDatosSolicitud(response: any, store?: DatosDelSolicituteSeccionStateStore): Partial<DatosDelSolicituteSeccionState> {
        const PARTIAL = GuardarAdapter_260905.fromApiResponseDatosSolicitud(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
    ...
    static patchToStoreDomicilio(response: any, store?: DomicilioStore): Partial<DomicilioState> {
        const PARTIAL = GuardarAdapter_260905.fromApiResponseRepresentanteLegal(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }   
        return PARTIAL;
    ...
    static patchToStoreManifestos(response: any, store?: DatosDomicilioLegalStore): Partial<DatosDomicilioLegalState> {
        const PARTIAL = GuardarAdapter_260905.fromApiResponseManifesto(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
    ...
    static obtenerParteEntera(valor: any): number {
    return Number.isFinite(Number(valor)) ? Math.trunc(Number(valor)) : 0;
    }
}