...
export class GuardarAdapter_260902 {
    /**
     ...
     */
    static toFormPayload(state: Tramite260902State, datosState: DatosDelSolicituteSeccionState, manifestoState: DatosDomicilioLegalState, representanteLegalDatos: DomicilioState): unknown {
    ...
    /**
         ...
         */
    // eslint-disable-next-line complexity
    static fromApiResponse(response: unknown): Partial<Tramite260902State> {
        ...
        const PARTIAL: Partial<Tramite260902State> = {
    ...
    private static mapSolicitudDatos(
        solicitud: any
    ): Partial<DatosDelSolicituteSeccionState> {
     ...
     * @returns Partial<Tramite260902State> (and patches the store when provided)
     */
    static patchToStore(response: any, store?: Tramite260902Store): Partial<Tramite260902State> {
        const PARTIAL = GuardarAdapter_260902.fromApiResponse(response);
        if (store) {
    ...
    static patchToStoreDatosSolicitud(response: any, store?: DatosDelSolicituteSeccionStateStore): Partial<DatosDelSolicituteSeccionState> {
        const PARTIAL = GuardarAdapter_260902.fromApiResponseDatosSolicitud(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
    ...
    static patchToStoreDomicilio(response: any, store?: DomicilioStore): Partial<DomicilioState> {
        const PARTIAL = GuardarAdapter_260902.fromApiResponseRepresentanteLegal(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
    ...
    static patchToStoreManifestos(response: any, store?: DatosDomicilioLegalStore): Partial<DatosDomicilioLegalState> {
        const PARTIAL = GuardarAdapter_260902.fromApiResponseManifesto(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
