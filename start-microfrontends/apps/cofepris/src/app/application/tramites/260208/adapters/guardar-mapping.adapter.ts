...
export class GuardarMappingAdapter {
  ...
  /**
   * Map an API response (form payload) back into a partial Tramite260208State.
   ...
   * @returns Partial<Tramite260208State>
   */
  // eslint-disable-next-line complexity
  static fromApiResponse(response: unknown): Partial<Tramite260208State> {
      ...
      const PARTIAL: Partial<Tramite260208State> = {
   ...
   * @param store Optional Tramite260208Store instance to apply the patch
   * @returns Partial<Tramite260208State> (and patches the store when provided)
   */
  static patchToStore(response: any, store?: Tramite260208Store): Partial<Tramite260208State> {
