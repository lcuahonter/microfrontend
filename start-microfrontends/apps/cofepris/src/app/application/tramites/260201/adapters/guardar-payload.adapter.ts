...
export class GuardarAdapter_260201 {
 ...
 private static mapScianData(arr?: any[]): any[] {
    ...
    /**
     * Map an API response (form payload) back into a partial Tramite260201State.
     ...
     * @returns Partial<Tramite260201State>
     */
    // eslint-disable-next-line complexity
    static fromApiResponse(response: unknown): Partial<Tramite260201State> {
        ...
        const PARTIAL: Partial<Tramite260201State> = {
            idSolicitud: RESP.idSolicitud ? Number(RESP.idSolicitud) : 0,
     ...
     * @param store Optional Tramite260201Store instance to apply the patch
     * @returns Partial<Tramite260201State> (and patches the store when provided)
     */
    static patchToStore(response: any, store?: Tramite260201Store): Partial<Tramite260201State> {
        const PARTIAL = GuardarAdapter_260201.fromApiResponse(response);
        if (store) {
