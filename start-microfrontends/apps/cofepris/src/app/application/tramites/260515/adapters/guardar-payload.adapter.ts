...
export class GuardarAdapter_260515 {
  ...
  : [],
     ...
     idSolicitud: GuardarAdapter_260515.idSolicitud
    };
  }
       ...
       * @param store Optional Tramite260515Store instance to apply the patch
       ...
       */
       ...
       static patchToStoreEstable(response: any, store?: AvisocalidadStore): void {
              const PARTIAL = GuardarAdapter_260515.fromApiResponseToEstablecimientoDatos(response);
              if (store) {
       ...
       static patchToStoreSolicitud(response: any, store?: DatosDomicilioLegalStore): void {
              const PARTIAL = GuardarAdapter_260515.fromApiResponseToSolicitudDatos(response);
              if (store) {
                  // Akita's update accepts a partial or updater function
                  store.update((state) => ({ ...state, ...PARTIAL }));
              }
             ...
             static patchToStorePago(response: any, store?: PagoDerechosStore): void {
              const PARTIAL = GuardarAdapter_260515.fromApiResponseToPago(response);
              if (store) {
                  // Akita's update accepts a partial or updater function
                  store.update((state) => ({ ...state, ...PARTIAL }));
              }
       ...
       static patchresponseToStore(response: any, avisocalidadStore: AvisocalidadStore, solicitudStore: DatosDomicilioLegalStore, pagoStore: PagoDerechosStore): void {
              GuardarAdapter_260515.patchToStoreEstable(response, avisocalidadStore);
              GuardarAdapter_260515.patchToStoreSolicitud(response, solicitudStore);
              GuardarAdapter_260515.patchToStorePago(response, pagoStore);
              GuardarAdapter_260515.idSolicitud=Number((response as any).idSolicitud);
          }
