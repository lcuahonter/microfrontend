...
export class GuardarAdapter_260915 {
  static toFormPayload(DatosDelSolicitudDatos:DatosDelSolicituteSeccionState ,manifestoDatos:DatosDomicilioLegalState,representanteLegalDatos:DomicilioState,terceros_derechosDatos:Solicitud260915State): unknown {
    return {
        "solicitante": {
            "rfc": "AAL0409235E6",
    ...
    /**
      ...
      static fromApiResponse(response: unknown): Partial<Solicitud260915State> {
          ...
          const PARTIAL: Partial<Solicitud260915State> = {
   ...
   /**
       ...
       */
      static patchToStore(response: any, store?:Solicitud260915Store): Partial<Solicitud260915State> {
          const PARTIAL = GuardarAdapter_260915.fromApiResponse(response);
          if (store) {
    ...
    private static mapEstablecimientoDatos(establecimiento: any): Partial<DatosDelSolicituteSeccionState> {
      ...
      static patchToStoreDatosSolicitud(response: any, store?: DatosDelSolicituteSeccionStateStore): Partial<DatosDelSolicituteSeccionState> {
          const PARTIAL = GuardarAdapter_260915.fromApiResponseDatosSolicitud(response);
          if (store) {
              store.update((state) => ({ ...state, ...PARTIAL }));
          }
          return PARTIAL;
    ...
    static patchToStoreDomicilio(response: any, store?: DomicilioStore): Partial<DomicilioState> {
        const PARTIAL = GuardarAdapter_260915.fromApiResponseRepresentanteLegal(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }   
        return PARTIAL;
    ...
    static patchToStoreManifestos(response: any, store?: DatosDomicilioLegalStore): Partial<DatosDomicilioLegalState> {
        const PARTIAL = GuardarAdapter_260915.fromApiResponseManifesto(response);
        if (store) {
            store.update((state) => ({ ...state, ...PARTIAL }));
        }
        return PARTIAL;
