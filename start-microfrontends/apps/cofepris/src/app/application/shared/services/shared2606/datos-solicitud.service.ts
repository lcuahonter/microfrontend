...
export class DatosSolicitudService {
  ...
  getRepresentanteLegala(body: Record<string, unknown>, idProcedimiento: string): Observable<JSONResponse> {
      return this.httpServicios.post<JSONResponse>(RFC_BUSCAR_REPRESENTANTE_LEGAL(idProcedimiento), body).pipe(
        map((response) => response)
      );
    }

  getCURP(curpValor: string, idProcedimiento: string): Observable<JSONResponse> {
      return this.httpServicios.post<JSONResponse>(OBTENER_CURP(idProcedimiento, curpValor), {}).pipe(
        map((response) => response)
      );
    }
