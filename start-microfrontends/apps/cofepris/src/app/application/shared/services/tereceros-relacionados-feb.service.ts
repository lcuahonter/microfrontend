...
import { Catalogo, CatalogoServices, Otros } from '@libs/shared/data-access-user/src';
...
export class TercerosRelacionadosFebService {
   ...
   */
  getPaisData(tramiteID: string): Observable<Catalogo[]> {
    return this.catalogoService.paisesCatalogo(tramiteID).pipe(
              map(res => res?.datos ?? [])
            );
  }
   ...
   */
  getMunicipioData(tramiteID: string, claveEstado: string): Observable<Catalogo[]> {
    return this.catalogoService.municipiosDelegacionesCatalogo(tramiteID, claveEstado).pipe(
      map(res => res?.datos ?? [])
    );
  }
   ...
   */
  getCodigoPostalData(tramiteID: string, claveEstado: string): Observable<Catalogo[]> {
    return this.catalogoService.codigoCatalogo(tramiteID, claveEstado).pipe(
      map(res => res?.datos ?? [])
    );
  }
   ...
   */
  getColoniaData(tramiteID: string, claveEstado: string): Observable<Catalogo[]> {
    return this.catalogoService.coloniasCatalogo(tramiteID, claveEstado).pipe(
      map(res => res?.datos ?? [])
    );
  }
   ...
   */
  getLocalidadData(tramiteID: string, claveEstado: string): Observable<Catalogo[]> {
    return this.catalogoService.localidadesCatalogo(tramiteID, claveEstado).pipe(
      map(res => res?.datos ?? [])
    );
  }
