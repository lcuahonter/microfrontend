...
export function importePagoValidator(control: AbstractControl): ValidationErrors | null {
  ...
  // Permitir vacío — el validador de requerido manejará los campos vacíos
  if (VALUE === null || VALUE === '') {
    return null;
  }

  // ✅ 1. Verificar si el valor es numérico
  const NUMERIC_REGEX = REG_X.SOLO_NUMEROS_Y_PUNTO;
  if (!NUMERIC_REGEX.test(VALUE)) {
    return { nonNumeric: true };
  }

  // ✅ 2. Verificar formato válido (máximo 15 dígitos y hasta 2 decimales)
  const DECIMAL_REGEX = REGEX_NUMERO_DECIMAL_DIGITOS;
...
setValoresStoreObject(
  ...
  static generarCatalogoObjeto(catalogo: Catalogo[], id: string): Catalogo[] | undefined {
    ...
    /**
         * Obtiene la respuesta de un catálogo desde una URL y la asigna a una variable específica del servicio.
         * @param url La URL del catálogo a obtener.
         * @param asignarA El nombre de la variable del servicio donde se asignará la respuesta.
         */
   ...
   * 
   * @returns 
    * Método que verifica si todos los campos del formulario tienen un valor válido.
   */
  ...
  /**
   * Método que convierte el valor del campo "llavePago" a mayúsculas
   */
  llavePagoCase(): void {
...
/**
 * 
 * Validador personalizado para campos decimales que limita el número de decimales permitidos.
 * @param maxDecimals 
 * @returns 
 */
