...
@Component({
  selector: 'app-generar-dictamen-2604',
  standalone: true,
  ...
  templateUrl: './generar-dictamen-2604.component.html',
  styleUrl: './generar-dictamen-2604.component.scss',
})
export class GenerarDictamen2604Component implements OnInit, OnChanges, OnDestroy,OnChanges {
    /**
   ...
   * @description Título del componente "Generar Dictamen".
   * Por defecto, se establece en "Generar Dictamen".
   */
  ...
  */
  ...
  ngOnInit(): void {
    ...
    if (this.soloLectura) {
      ...
      this.dictamenForm.get('opinion')?.disable();
      this.dictamenForm.get('justificacion')?.disable();
      this.dictamenForm.get('descripcionUsoAutorizado')?.disable();
    }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['dataIniciarDictamen'] && changes['dataIniciarDictamen'].currentValue && this.dictamenForm) {
      ...
      this.dictamenForm.patchValue({
        aceptada: this.dataIniciarDictamen?.aceptada??false,
        ...
        descripcionUsoAutorizado: this.dataIniciarDictamen?.descripcion_uso_autorizado || this.dataIniciarDictamen?.descripcionUsoAutorizado,
        ...
        plazoVigencia: this.dataIniciarDictamen?.plazo_vigencia?.toString() || this.dataIniciarDictamen?.plazoVigencia?.toString(),
        siglasDictaminador: this.dataIniciarDictamen?.siglas_dictaminador || this.dataIniciarDictamen?.siglasDictaminador,
        numeroGenerico1: this.dataIniciarDictamen?.numero_generico_1?.toString() || this.dataIniciarDictamen?.numeroGenerico1?.toString(),
        idMotivoTipoTramite: this.dataIniciarDictamen?.idMotivoTipoTramites?.[0] ?? null,
        fechaFinVigencia: GenerarDictamen2604Component.formatDate(this.dataIniciarDictamen?.fecha_fin_vigencia),
      });
    ...
    if (this.soloLectura) {
      this.dictamenForm.get('opinion')?.disable({ emitEvent: false });
      this.dictamenForm.get('justificacion')?.disable({ emitEvent: false });
      this.dictamenForm.get('descripcionUsoAutorizado')?.disable({ emitEvent: false });
    } 
  }
  ...
  InitDictamenForm():void
  {
    this.dictamenForm = this.fb.group({
      ...
      justificacionNegativa: ['', [
        Validators.required,
        GenerarDictamen2604Component.noSoloEspacios,
...
onAceptadaChange(event: Event) :void {
  ...
  private actualizarVisibilidadCamposFecha(valorCumplimiento: string): void {
    ...
    if (ESDICTAMENACEPTADO) {
      this.updateFieldValidators([ 'descripcionUsoAutorizado',
        'opinion', 'justificacion', 'plazoVigencia','fechaFinVigencia']);
    } else {
      this.clearFieldValidators([ 'descripcionUsoAutorizado', 'opinion', 'justificacion', 'plazoVigencia','fechaFinVigencia'])
    }
    ...
    if (ESDICTAMENPARCIAL) {
      ...
      this.updateFieldValidators([ 'descripcionUsoAutorizado', 'opinion', 'justificacionNegativa', 'idMotivoTipoTramite',
