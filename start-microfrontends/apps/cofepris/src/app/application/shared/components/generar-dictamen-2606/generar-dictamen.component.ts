...
/**
 * @component
 * @name GenerarDictamenComponent2606
 ...
 */
@Component({
  selector: 'app-generar-dictamen-2606',
  standalone: true,
...
export class GenerarDictamenComponent2606 implements OnInit, OnChanges, OnDestroy {
  /**
  ...
  InitDictamenForm():void
  {
    this.dictamenForm = this.fb.group({
      ...
      justificacionNegativa: ['', [
        ...
        GenerarDictamenComponent2606.noSoloEspacios,
  ...
  private actualizarVisibilidadCamposFecha(valorCumplimiento: string): void {
   ...
   /*   this.updateFieldValidators(['usoAutorizadoDictamen', 'descripcionUsoAutorizado', 'idRestriccionTipoTramite',
    ...
    if (ESDICTAMENRECHAZADO) {
      this.updateFieldValidators([ 'idMotivoTipoTramite']);
      ...
      this.clearFieldValidators(['idMotivoTipoTramite']);
    }
  }
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['dataIniciarDictamen'] && changes['dataIniciarDictamen'].currentValue && this.dictamenForm) {
      ...
      this.dictamenForm.patchValue({
        ...
        fechaFinVigencia: GenerarDictamenComponent2606.formatDate(this.dataIniciarDictamen?.fecha_fin_vigencia),
      });
