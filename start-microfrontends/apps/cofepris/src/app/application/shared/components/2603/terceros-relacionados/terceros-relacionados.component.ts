...
private static generateConfiguracionTabla<T>(
  ...
  public abrirModalParaModificar(titulo: string, datosExistentes: Fabricante | Otros260303, tipoTabla: string): void {
    ...
    if (content?.tableValidEvent) {
  content.tableValidEvent.subscribe((value: string) => {
   ...
   */
private actualizarTablaConDatosModificados(
  tipoTabla: string,
  datosOriginales: Fabricante | Otros260303,
  datosActualizados: Fabricante | Otros260303
): void {
  ...
  const updateArray = <T>(
    ...
    const index = array.findIndex(item => item === original);
  ...
  switch (tipoTabla) {
    ...
    case 'fabricante': {
      this.fabricanteTablaDatos =
        updateArray(this.fabricanteTablaDatos, datosOriginales as Fabricante, datosActualizados as Fabricante);

      this.tramite2603Store.setFabricantes(this.fabricanteTablaDatos);
    ...
    case 'facturador': {
      this.facturadorTablaDatos =
        updateArray(this.facturadorTablaDatos, datosOriginales as Fabricante, datosActualizados as Fabricante);

      this.tramite2603Store.setFacturadores(this.facturadorTablaDatos);
      break;
    }

    case 'proveedor': {
      this.proveedorTablaDatos =
        updateArray(this.proveedorTablaDatos, datosOriginales as Fabricante, datosActualizados as Fabricante);

      this.tramite2603Store.setProveedores(this.proveedorTablaDatos);
      break;
    }

    case 'certificadoAnalitico': {
      this.certificadoAnaliticoTablaDatos =
        updateArray(this.certificadoAnaliticoTablaDatos, datosOriginales as Fabricante, datosActualizados as Fabricante);

      this.tramite2603Store.setCertificadosAnaliticos(this.certificadoAnaliticoTablaDatos);
      break;
    }

    case 'otros': {
      this.otrosTablaDatos =
        updateArray(this.otrosTablaDatos, datosOriginales as Otros260303, datosActualizados as Otros260303);

      this.tramite2603Store.setOtros(this.otrosTablaDatos);
...
manejarConfirmacionNotificacion(confirmar: boolean): void {
  ...
  if (!confirmar || !this.pendingDeleteType) {
  ...
  switch (this.pendingDeleteType) {
    ...
    case 'fabricante':
      this.fabricanteTablaDatos = this.fabricanteTablaDatos.filter(
        f => !this.selectedFabricanteRows.includes(f)
      );
      this.tramite2603Store.setFabricantes(this.fabricanteTablaDatos);
      this.selectedFabricanteRows = [];
      break;
    ...
    case 'facturador':
      this.facturadorTablaDatos = this.facturadorTablaDatos.filter(
        f => !this.selectedFacturadorRows.includes(f)
      );
      this.tramite2603Store.setFacturadores(this.facturadorTablaDatos);
      this.selectedFacturadorRows = [];
      break;
    ...
    case 'proveedor':
      this.proveedorTablaDatos = this.proveedorTablaDatos.filter(
        p => !this.selectedProveedorRows.includes(p)
      );
      this.tramite2603Store.setProveedores(this.proveedorTablaDatos);
      this.selectedProveedorRows = [];
      break;
    ...
    case 'certificadoAnalitico':
      this.certificadoAnaliticoTablaDatos = this.certificadoAnaliticoTablaDatos.filter(
        c => !this.selectedCertificadoAnaliticoRows.includes(c)
      );
      this.tramite2603Store.setCertificadosAnaliticos(this.certificadoAnaliticoTablaDatos);
      this.selectedCertificadoAnaliticoRows = [];
      break;
    ...
    case 'otros':
      this.otrosTablaDatos = this.otrosTablaDatos.filter(
        o => !this.selectedOtrosRows.includes(o)
      );
      this.tramite2603Store.setOtros(this.otrosTablaDatos);
      this.selectedOtrosRows = [];
      break;
  ...
  // Clear completely and show success message immediately
  this.nuevaNotificacion = {} as Notificacion;
