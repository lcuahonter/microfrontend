...
verificarAlerta:number[]=[];
   ...
   */
  markTouched(): void {
    if (this.isContinuarTriggered && this.solicitudState?.claveReferencia !== '') {
...
onBancoSeleccionado(event: any): any {
  ...
  const SELECTED = BANCO_OBJ ? BANCO_OBJ[0] : undefined;
  if (SELECTED) {
    this.pagoDerechosForm.patchValue({ banco: SELECTED.clave });
    this.pagoDerechosForm.get('banco')?.markAsTouched();
    this.pagoDerechosForm.get('banco')?.markAsDirty();
    this.setValoresStore(this.pagoDerechosForm, 'banco', 'setBanco');
    this.setValoresStoreObject(SELECTED, 'banco', 'setBancoObject');
  }
