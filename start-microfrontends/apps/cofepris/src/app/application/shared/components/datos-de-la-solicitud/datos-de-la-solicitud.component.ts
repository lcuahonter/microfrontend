...
public mercanciaSeleccionada: TablaMercanciasDatos | undefined;
 ...
 /**
  * Indica si la acción actual es modificar.
  */
...
if (this.tablaMercanciasConfig) {
        this.tablaMercanciasConfig.tipoSeleccionTabla = seccionState.readonly
          ? TablaSeleccion.UNDEFINED
          : TablaSeleccion.CHECKBOX;
      }
            })
      )
      .subscribe();
 ...
 */
 ...
 agregarMercancias(): void {
  ...
  alternarControlesDeFormulario(enable: boolean): void {
   ...
   if(this.idProcedimiento!==260209 && this.idProcedimiento!==260210&& this.idProcedimiento!==260205 && this.idProcedimiento!==260213 && this.idProcedimiento!==260207){
  ...
  private habilitarCamposFormulario(): void {
    ...
    Object.keys(this.datosSolicitudForm.controls).forEach((controlName) => {
      ...
      if (controlName === 'estado'||this.idProcedimiento===260209 || this.idProcedimiento===260210 || this.idProcedimiento===260213) {
        return;
      }
...
marcarTodosLosCamposComoTocados(): void {
  ...
  // Campos adicionales a excluir SOLO para el procedimiento 260208
  const EXCLUDED_FIELDS_260208_ONLY = [
    'regimen',
    'adunasDeEntradas',
    'aeropuerto',
    'representanteRfc',
  ...
  let EXCLUDED_FIELDS = [...EXCLUDED_FIELDS_COMMON];
  
  if (this.idProcedimiento === 260208 || this.idProcedimiento === 260213) {
    EXCLUDED_FIELDS = [...EXCLUDED_FIELDS, ...EXCLUDED_FIELDS_260208_ONLY];
  }
  
  // Siempre excluir rfcSanitario a menos que sea el procedimiento 260203
  if (this.idProcedimiento !== 260203) {
    EXCLUDED_FIELDS.push('rfcSanitario');
  }
...
verificarCamposValidosODeshabilitados(): boolean {
  ...
  // Campos excluidos comunes para TODOS los procedimientos
  const EXCLUDED_FIELDS_COMMON = [
    'aduana',
    'aeropuertoDos',
    'calleYNumero',
  ...
  const EXCLUDED_FIELDS_260208_ONLY = [
   ...
   const EXCLUDED_FIELDS_260215_ONLY = [
    "aeropuerto",
    "representanteNombre",
    "apellidoPaterno",
    "apellidoMaterno"
  ...
  if(this.idProcedimiento===260215){
    EXCLUDED_FIELDS = [...EXCLUDED_FIELDS, ...EXCLUDED_FIELDS_260215_ONLY];
  }
...
onMercanciaSeleccionado(mercanciaData: TablaMercanciasDatos): void {
    ...
    item => item.id === this.mercanciaSeleccionada!.id
      ...
      this.tablaMercanciasConfig.datos = this.tablaMercanciasConfig.datos.map(
      (item, i) => i === INDEX ? { ...mercanciaData } : item
    );
      }
