...
export class DatosDeLaSolicitudComponent
  ...
  */
  ...
  ngOnInit(): void {
    ...
    this.etiquetaMunicipio =
      this.idProcedimiento === NUMERO_TRAMITE.TRAMITE_260103 || this.idProcedimiento === NUMERO_TRAMITE.TRAMITE_260704
  ...
  onRadioChange(event: string | number): void {
    ...
    const JUSTIFICATION_CTRL = this.datosSolicitudForm.get('justification');
  ...
  */
  ...
  crearDatosSolicitudForm(): void {
    const TIPOPERACIONVALUE = this.datosSolicitudFormState?.tipoOperacion;
    this.datosSolicitudForm = this.fb.group({
      tipoOperacion: [
        { value: TIPOPERACIONVALUE, disabled: false }
      ],
      justification: [
        {
          value: this.datosSolicitudFormState?.justification,
          disabled: !(TIPOPERACIONVALUE === 'modificacion' || TIPOPERACIONVALUE === 'modificacion_prorroga')
        },
      ...
      immex: [
        {
          value: this.datosSolicitudFormState?.immex ?? '',
          disabled: false,
        },
        []
      ],
      ano: [
        {
          value: this.datosSolicitudFormState?.ano ?? '',
          disabled: false,
        },
        []
      ],
  ...
  ngAfterViewInit(): void {
   ...
   /**
   * Sets the enabled/disabled state of the 'justificacion' field based on the radio selection.
   * Call this after form creation and whenever the radio value or form state changes.
   */
  private setJustificacionState(): void {
    if (!this.datosSolicitudForm){
      return;
    } 
    const RADIOVALOR = this.datosSolicitudForm.get('tipoOperacion')?.value;
    const JUSTIFICACIONCONTROL = this.datosSolicitudForm.get('justification');
    if (!JUSTIFICACIONCONTROL){
      return;
    }
    if (RADIOVALOR === 'modificacion' || RADIOVALOR === 'modificacion_prorroga') {
      JUSTIFICACIONCONTROL.enable({ emitEvent: false });
    } else {
      JUSTIFICACIONCONTROL.disable({ emitEvent: false });
    }
  }
 ...
 ngOnChanges(changes: SimpleChanges): void {
  ...
  if (
    changes['datosSolicitudFormState'] &&
    ...
    const PATCH = {
      ...
      ...changes['datosSolicitudFormState'].currentValue,
      immex: changes['datosSolicitudFormState'].currentValue?.immex ?? '',
      ano: changes['datosSolicitudFormState'].currentValue?.ano ?? '',
      regimen: changes['datosSolicitudFormState'].currentValue?.regimen ?? ''

    };
    this.datosSolicitudForm.patchValue(PATCH, { emitEvent: false });
    this.cambioAviso();
 ...
 */
  ...
  */
  onMercanciaSeleccionado(mercanciaData: TablaMercanciasDatos): void {
    console.log('Mercancía seleccionada:', mercanciaData);
