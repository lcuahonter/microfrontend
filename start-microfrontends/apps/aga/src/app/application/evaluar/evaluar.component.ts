...
import { CatalogoTipoDocumento, CategoriaMensaje, ConsultaioQuery, ConsultaioState, ConsultaioStore, DocumentosEspecificosResponse, FECHA_DE_INICIO, Notificacion, NotificacionesComponent, base64ToHex, encodeToISO88591Hex } from '@ng-mf/data-access-user';
...
import { ValidacionDocumentosRequerimiento } from '../shared/models/validacion-documentos-requerimiento';
...
export class EvaluarComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  /**
   * Mensaje a mostrar si se guardan correctamente los requisitos.
   */
  msgRequerimientoGuardado!: string;
 ...
 */
  ...
  /**
   * @property {boolean} validacionJustificacionRequerimiento
   * @description Indica si hay error en la validación de justificación para mostrar mensaje en el componente.
   */
  validacionJustificacionRequerimiento = false;
 ...
 */
  ...
  if (
    ...
    if ( this.justificacion === null || this.justificacion.trim() === '') {
     ...
     this.validacionJustificacionRequerimiento = true;
     // Regresar a la pestaña de capturar requerimiento
     this.indiceDictamen = 1;
     ...
     this.nuevaNotificacion = {
       ...
       mensaje: 'No cumple con las validaciones del modelo',
       cerrar: false,
       txtBtnAceptar: '',
       txtBtnCancelar: '',
     };
   ...
   this.validacionJustificacionRequerimiento = false;
}
  ...
  onFormRequerimientoChanged(formValue: any) {
    ...
    if (this.validacionJustificacionRequerimiento && this.justificacion && this.justificacion.trim() !== '') {
      this.validacionJustificacionRequerimiento = false;
    }
  }
  ...
  guardarRequerimiento(): void {
    ...
    // Validar justificación antes de continuar
    ...
    if (CONFIG.TablaDocumentosRequeridos && this.alcanceRequerimiento !== this.REQ_TIPO_DATOS) {
      // Validar si hay documentos en la tabla
      const TIENE_DOCUMENTOS = this.listaDocumentosRequeridos.length > 0 || 
                              this.dataIniciarRequerimiento.documentos.length > 0;
      
      const TIENE_DOCUMENTOS_ACTIVOS = DOCUMENTOSPARAGUARDAR.length > 0;
      
      if (!TIENE_DOCUMENTOS) {
        this.validacionDocumentos = true;
        this.indiceDictamen = 2; // Cambiar a la pestaña Seleccionar documentos
        this.validarJustificacionRequerimiento();
        return;
      }
      ...
      .subscribe({
        next: (resp) => {
          if (resp.codigo === CodigoRespuesta.EXITO) {
            this.firmar = true;
            window.scrollTo({ top: 0, behavior: 'smooth' });
            this.msgRequerimientoGuardado = 'Requisitos guardados correctamente.'
            this.cadenaOriginalRequerimiento = resp.datos?.cadena_original || '';
            setTimeout(()=> {
              this.msgRequerimientoGuardado = '';
            },5000);
          } else {
            window.scrollTo({ top: 0, behavior: 'smooth' });
            this.nuevaNotificacion = {
        ...
        error: (err) => {
          ...
          this.nuevaNotificacion = {
            ...
            categoria: CategoriaMensaje.ERROR,
            modo: 'action',
            titulo: err?.error?.error || 'Error en los datos.',
            mensaje: MENSAJE,
            cerrar: false,
            txtBtnAceptar: '',
            txtBtnCancelar: '',
