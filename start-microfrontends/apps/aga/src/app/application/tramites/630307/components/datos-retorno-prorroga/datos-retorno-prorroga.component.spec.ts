...
describe('DatosRetornoProrrogaComponent', () => {
  ...
  it('debería obtener el estado actual del store', () => {
    ...
    expect(componente.estadoSeleccionado).toEqual({ campo: 'valorRetornoProrroga' });
  });

  it('debería actualizar el store cuando establecerCambioDeValor es llamado', () => {
    const eventoMock = { campo: 'campoRetornoProrroga', valor: 'valorRetornoProrroga' };

    componente.establecerCambioDeValor(eventoMock);

    expect(storeMock.setTramite630307State).toHaveBeenCalledWith('campoRetornoProrroga', 'valorRetornoProrroga');
  });
