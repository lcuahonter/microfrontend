...
import { CategoriaMensaje, Notificacion, NotificacionesComponent, REGEX_CURP, TablaDinamicaComponent, TablaSeleccion, TipoNotificacionEnum, TituloComponent } from '@libs/shared/data-access-user/src';
 ...
 */
  ...
  /**
   * Checks if a string contains only digits (for template validation)
   */
 public isAllDigits(value: string): any {
  ...
  crearFormulario(): void {
    this.formularioAgentesAduanales = this.fb.group({
      id: [null],
      // Only letters (including accents), numbers, and spaces allowed, min 2, max 100 chars
      nombreAgente: ['', [
        Validators.required,
        Validators.pattern(/^[A-Za-zÁÉÍÓÚáéíóúÑñ0-9 ]{2,100}$/)
      ]],
      primerApellido: ['',  [
        Validators.required,
        Validators.pattern(/^[A-Za-zÁÉÍÓÚáéíóúÑñ0-9 ]{2,100}$/)
