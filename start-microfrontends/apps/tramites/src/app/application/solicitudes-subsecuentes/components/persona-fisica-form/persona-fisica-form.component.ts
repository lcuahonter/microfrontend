...
import { ConfiguracionColumna, REGEX_RFC_FISICA, TablaDinamicaComponent, TablePaginationComponent } from '@ng-mf/data-access-user';
...
@Component({
  selector: 'app-persona-fisica-form',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, TablaDinamicaComponent, TablePaginationComponent],
  templateUrl: './persona-fisica-form.component.html'
})
export class PersonaFisicaFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.form = this.fb.group({
      rfc: [null, [Validators.required, Validators.pattern(REGEX_RFC_FISICA)]]
    });
  ...
  /**
   * Busca solicitudes para el tipo de persona persona fisica
   */
  onBuscar(): void {
