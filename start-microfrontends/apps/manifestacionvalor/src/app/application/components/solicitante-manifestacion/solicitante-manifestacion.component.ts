...
import { SolicitanteEvaluarResponse } from '@libs/shared/data-access-user/src/core/models/datos-solicitante-evaluar.model';


@Component({
  selector: 'solicitante-manifestacion',
  standalone: true,
  imports: [
    TituloComponent,
    ReactiveFormsModule,
  ...
  templateUrl: './solicitante-manifestacion.component.html',
  styleUrl: './solicitante-manifestacion.component.scss',
  host: {}
})
export class SolicitanteManifestacionComponent implements OnInit, OnDestroy {
      ...
      .pipe(
        tap((response) => {
          if (response?.datos) {
            ...
            // Patch al formulario
            SolicitanteManifestacionComponent.patchValuesToForm(
            ...
            SolicitanteManifestacionComponent.patchValuesToForm(DATOS_TRAMITE_FORM, DATOS_TRAMITE_MAPPED);
  ...
  inicializarFormGroup(
    ...
    config.forEach((campo) => {
      const VALIDATORS = SolicitanteManifestacionComponent.getValidators(campo.validators);
      GRUPO.addControl(
        campo.campo,
