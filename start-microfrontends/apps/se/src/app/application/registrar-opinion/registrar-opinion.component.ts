...
import { CatalogoTipoDocumento, CategoriaMensaje, ConsultaioQuery, ConsultaioState, ConsultaioStore, DesplazarseHaciaArribaService, DocumentosEspecificosResponse, FECHA_DE_INICIO, Notificacion, NotificacionesComponent, TabEvaluarTratadosResponse, TabsConfigService, base64ToHex, encodeToISO88591Hex } from '@ng-mf/data-access-user';
...
import { RegistrarOpinion130Component } from '../shared/components/registrar-opinion-130/registrar-opinion-130.component';
import { RegistrarOpinionComponent801 } from '@libs/shared/data-access-user/src/tramites/components/registrar-opinion-801/registrar-opinion-801.component';
...
@Component({
  ...
  imports: [CommonModule, ReviewersTabsComponent,
    ...
    RegistrarOpinionComponent801,
    RegistrarOpinion130Component,
  ],
  templateUrl: './registrar-opinion.component.html',
  styleUrl: './registrar-opinion.component.scss',
})
export class RegistrarOpinionComponent implements OnInit, OnDestroy {
  /**
  ...
  */
  ...
  /**
   * @property {number} indiceOpinion
   * @description Índice de la pestaña de opinión seleccionada.
   */
 ...
 */
      ...
      .subscribe({
        next: (response) => {
          if (response.codigo === CodigoRespuesta.EXITO) {
            ...
            if(this.tramite === 80101 && this.opcionesDisponibles.length > 1){
   ...
   * @returns {void}
   */
  firmaOpinion(firma: string): void {
    ...
    const PAYLOAD = {
      ...
      firma: {
        ...
        fecha_firma: RegistrarOpinionComponent.formatFecha(new Date()),
        clave_rol: 'Dictaminador',
        sello: FIRMAHEX,
      }
    };

    this.opinionService.postopinionregistrarFirmar(this.tramite, NUMFOLIO, PAYLOAD)
      .pipe(
        takeUntil(this.destroyNotifier$),
        tap((firmaResponse: BaseResponse<null>) => {
          if (firmaResponse.codigo !== '00') {
            ...
            localStorage.setItem('mensajeExito', 'REGISTRO');
...
onDocumentosRequeridos(lista: Documentos[]): void {
  ...
  guardarRequerimiento(): void {
   ...
   if (this.requerimientoConfig.isSegundaTabla === true && this.dataIniciarRequerimiento.documentos.length > 0) {
    ...
    if (this.requerimientoConfig.isSegundaTabla === true) {
      ...
      this.validarJustificacionRequerimiento();
    } else if (this.requerimientoConfig.isSegundaTabla === false) {
