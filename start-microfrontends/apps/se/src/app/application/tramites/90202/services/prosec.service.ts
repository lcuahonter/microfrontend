...
export class ProsecService {
    /**
     * Obtiene la descripción de la actividad productiva desde el endpoint externo usando API_ROUTES.
     * @param idSolicitud
     * @param idProgramaAutorizado
     */
    obtenerActividadProductiva(idSolicitud: string, idProgramaAutorizado: string): Observable<JSONResponse> {
      const URL = API_ROUTES('/sat-t90202', '90202').obtenerActividadProductiva(idSolicitud, idProgramaAutorizado);
      return this.http.get<JSONResponse>(URL).pipe(
        map((response) => response)
      );
    }
    /**
     * Obtiene la representación federal desde el endpoint externo usando API_ROUTES.
     * @param idSolicitud
     * @param idProgramaAutorizado
     * @param fechaProsec
     */
    obtenerRepresentacionFederal(idSolicitud: string, idProgramaAutorizado: string, fechaProsec: string): Observable<JSONResponse> {
      const URL = API_ROUTES('/sat-t90202', '90202').obtenerRepresentacionFederal(idSolicitud, idProgramaAutorizado, fechaProsec);
      return this.http.get<JSONResponse>(URL).pipe(
        map((response) => response)
      );
    }
  ...
  obtenerEstadoTablaDatos(body: { idSolicitud: string; idProgramaAutorizado: number }): Observable<JSONResponse> {
    return this.http.post<JSONResponse>(API_ROUTES('/sat-t90202', '90202').buscarPlanta, body).pipe(
      map((response) => response),
      catchError(() => {
  ...
  obtenerFraccionesTablaDatos(body: { fraccion: string; id_conf_programa_se: string; cve_sector: string; id_programa_autorizado: string | null }): Observable<JSONResponse> {
    return this.http.post<JSONResponse>(API_ROUTES('/sat-t90202', '90202').buscarSectorFraccionArancelaria, body).pipe(
      map((response) => response),
      catchError(() => {
        const ERROR = new Error(`Error al obtener información de las fracciones en ${API_ROUTES('/sat-t90202', '90202').buscarSectorFraccionArancelaria}`);
        return throwError(() => ERROR);
      })
    );
  }
