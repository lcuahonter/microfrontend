...
export class SectoresYMercanciasComponent implements OnInit, OnDestroy {
  ...
  producirColumnConfiguracion: ConfiguracionColumna<FilaProducir>[] = [
    { encabezado: 'Fracción arancelaria', clave: (fila) => fila.cveFraccion, orden: 1 },
  ...
  espectaculoDuplicateAlerta: boolean = false;
  /**
      ...
      if (esValidObject(response)) {
        ...
        if (
          ...
          const NUEVO_SECTOR: FilaSectors = {
            sector: SECTOR_API.sector,
            cvSectorCatalogo: SECTOR_API.cvSectorCatalogo,
            idConfProgramaSE:SECTOR_API.idConfProgramaSE
          };
      ...
      next: (response) => {
        if (esValidObject(response)) {
          ...
          if (
            ...
            if (API_DATOS.datos.fraccion_seleccionada.length === 0) {
              ...
              this.nuevaNotificacion = {
                ...
                modo: 'action',
                titulo: '',
                mensaje: API_DATOS?.datos?.response || 'No se encontraron datos para la fracción arancelaria.',
                cerrar: false,
                tiempoDeEspera: 2000,
            ...
            const NUEVA_FRACCION: FilaProducir = {
              cveFraccion: FRACCION_API.fraccionArancelaria.cveFraccion,
              cveSector: FRACCION_API.cveSector,
              idConfProgramaSE:FRACCION_API.sector.idConfProgramaSE
            };
...
eliminarDuplicate(): void {
  this.espectaculoDuplicateAlerta = false;
}
  ...
  if (
    this.sectors &&
    this.sectors.some(sector => sector.cvSectorCatalogo === CVE_SECTORES)
  ){
    this.espectaculoDuplicateAlerta = true;
    this.nuevaNotificacion = {
      tipoNotificacion: 'alert',
      categoria: 'danger',
      modo: 'action',
      titulo: '',
      mensaje: 'El sector que intenta ingresar ya existe en la lista de sectores capturados.',
      cerrar: false,
      tiempoDeEspera: 2000,
      txtBtnAceptar: 'Aceptar',
      txtBtnCancelar: '',
  ...
  agregarProducir(): void {
    if (!this.listSelectedView || this.listSelectedView.length === 0) {
  ...
  else if (this.sectoresYMercancias.get('Fraccion_arancelaria')?.value === '') {
    this.espectaculoAlerta = true;
      this.nuevaNotificacion = {
        tipoNotificacion: 'alert',
        categoria: 'danger',
