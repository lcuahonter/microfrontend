...
import { ConsultaioQuery, SeccionLibQuery, SeccionLibState, SeccionLibStore } from '@ng-mf/data-access-user';
import {
  FormBuilder,
  FormControl,
  FormGroup,
...
export class DestinatarioDeComponent implements OnDestroy, OnInit {
  ...
  /**
   * Detalles del transporte actuales.
   */
  detallesDelTransporte!: FormValues;
  /**
  ...
  inicializarSuscripciones(): void {
    ...
    this.tramiteQuery.selectDetallesDelTransporte$
      .pipe(takeUntil(this.destroyNotifier$))
      .subscribe((estado) => {
        this.detallesDelTransporte = estado as unknown as FormValues;
      });
  ...
  /**
   *  Actualiza los detalles del transporte en el estado de la tienda.
   * @param event  Objeto que contiene la información para actualizar los detalles del transporte.
   */
  setDetallesDelTransporteStore(event: {
    formGroupName: string;
    campo: string;
    valor: undefined;
    storeStateName: string;
    ...
    const { campo: CAMPO, valor: VALOR } = event;
    this.store.setDetallesDelTransporte({ [CAMPO]: VALOR });
  }
