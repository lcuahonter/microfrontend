...
*/
@Component({
  selector: 'app-exportador-autorizado-evaluar',
  standalone: true,
  imports: [CommonModule, TooltipModule, ReactiveFormsModule],
  templateUrl: './exportador-autorizado-evaluar.component.html',
  styleUrl: './exportador-autorizado-evaluar.component.scss'
})


export class ExportadorAutorizadoEvaluarComponent implements OnInit {
  ...
  */
   ...
   */
  public radioOpcionsExpAut = RADIO_OPCIONS;
    ...
    this.exportadorAutorizadoService.getExportadorAutorizado(this.consultaState.id_solicitud)
      ...
      .subscribe({
        next: (response) => {
          ...
          if (this.tipo === 'UE') {
            CLAVE = response.datos.clave_condicion_exportador;
            CALIFICACION = response.datos.calificacion_exp_aut_sistema;
          } else {
            CLAVE = response.datos.clave_condicion_exportador_jpn;
            CALIFICACION = response.datos.calificacion_exp_aut_sistema_jpn;
          }
