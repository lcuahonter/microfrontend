 ...
 */
  ...
  obtenerEstadoSolicitud(): void {
    this.tramite140101Query.selectSolicitud$?.pipe(takeUntil(this.destroyNotifier$))
      .subscribe((data: Programa140101State) => {
    ...
    cargarDatos(): void {
      ...
      .subscribe((data) => {
       ...
       if (this.programaState && this.programaState.programaACancelar) {
        const SELECTED_PROGRAM = this.programaState.programaACancelar;
        const INDEX = this.datosTabla.findIndex(
          (x) => x.idProgramaAutorizado === SELECTED_PROGRAM.idProgramaAutorizado
        );
        this.radioId = INDEX;
...
valorDeAlternancia(row: ProgramaACancelar): void {
  setTimeout(() => {
    ...
    this.radioId = INDEX; // Update the radioId
    this.tramite140101Store.setRadioSelection(INDEX); // Update the store
  ...
  }, 0); // Ensure this runs after the current change detection cycle
}
