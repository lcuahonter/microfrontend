import { API_POST_PARCHE_PRELLENADAS, COMUN_URL, HttpCoreService, JSONResponse, formatearFechaYyyyMmDd } from '@libs/shared/data-access-user/src';
...
export class CertificadosOrigenService {
  ...
  /**
   * URL base del host para las solicitudes HTTP del servicio.
   *
   * Esta propiedad almacena la dirección base utilizada para construir los endpoints
   * de las peticiones relacionadas con el certificado de origen.
   */
  host: string;
 ...
 */
  agregarProductores(body: {rfc_solicitante: string}): Observable<JSONResponse> {
    return this.httpService.post<JSONResponse>(PROC_110217.AGREGAR_PRODUCTOR, { body: body });
  }
  ...
  */
  ...
  buildDestinatario(data: Tramite110217State): unknown {
    return {
      ...
      "primer_apellido": data.grupoReceptor.primerApellido,
      "segundo_apellido": data.grupoReceptor.segundoApellido,
      "numero_registro_fiscal": data.grupoReceptor.numeroDeRegistroFiscal,
   ...
   * Este método realiza una solicitud HTTP GET para obtener los datos de consulta simulados desde el archivo.
   * 
