...
import { BtnContinuarComponent, Notificacion, NotificacionesComponent } from '@ng-mf/data-access-user';
...
import { REGEX_RFC_FISICA, REGEX_RFC_MORAL } from '@ng-mf/data-access-user';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, TituloComponent, BtnContinuarComponent, InputRadioComponent, AlertComponent, TableComponent, TablaDinamicaComponent, CatalogoSelectComponent,NotificacionesComponent],
...
/**
 * Contiene la información de la notificación a mostrar en pantalla.
 * Se utiliza para configurar alertas, mensajes y modales dentro del componente.
 */
...
agregarSocio(): void {
  const NACIONALIDAD = this.FormSolicitud.get(['datosGeneralesSocios', 'nacionalidad'])?.value;
  const TIPO_PERSONA = this.FormSolicitud.get(['datosGeneralesSocios', 'persona'])?.value;
  const RFC = this.FormSolicitud.get(['datosGeneralesSocios', 'cadenaDependencia'])?.value || '';

  if (!this.validarRFC(NACIONALIDAD, TIPO_PERSONA, RFC)) {
    return;
  }
  ...
  if (this.camposEntradaRegulares && RFC) {
    this.agregarSocioRegular(RFC);
  }
  if (this.camposPersonaMoralExtranjera) {
    this.agregarSocioMoralExtranjero(VALOR_FORMULARIO);
  }
  if (this.camposPersonaFisicaExtranjera) {
    this.agregarSocioFisicoExtranjero(VALOR_FORMULARIO);
  }
}

/**
 * Valida el RFC según la nacionalidad y el tipo de persona (física o moral).
 * Verifica el formato mediante expresiones regulares y la longitud correspondiente.
 * Muestra un mensaje de error y devuelve false si el RFC no cumple los criterios.
 */
private validarRFC(NACIONALIDAD: string, TIPO_PERSONA: string, RFC: string): boolean {
  if (NACIONALIDAD === 'Si' && TIPO_PERSONA === 'Si' && !REGEX_RFC_FISICA.test(RFC)) { // Persona física
    if (RFC.length !== 13) {
      this.mostrarPopupError('El RFC debe tener al menos 13 caracteres de longitud');
      return false;
    }
  } else if (NACIONALIDAD === 'Si' && TIPO_PERSONA === 'No' && !REGEX_RFC_MORAL.test(RFC)) { // Persona moral
    if (RFC.length !== 12) {
      this.mostrarPopupError('El RFC debe tener al menos 12 caracteres de longitud');
      return false;
    }
  }
  return true;
...
/**
 * Consulta los datos de un socio mediante su RFC y los agrega al registro.
 * Llama al servicio correspondiente y aplica la respuesta recibida.
 * Controla el estado de carga y finaliza la suscripción al destruir el componente.
 */
private agregarSocioRegular(RFC: string): void {
...
/**
 * Agrega un nuevo socio extranjero moral a la lista usando los datos del formulario.
 * Construye el objeto con valores normalizados y obtiene la descripción del país.
 * Actualiza la tabla de socios extranjeros reflejando el nuevo registro.
 */
private agregarSocioMoralExtranjero(VALOR_FORMULARIO: any): void {
...
/**
 * Agrega un nuevo socio extranjero persona física utilizando los datos del formulario.
 * Construye el registro con la información capturada y convierte el país a su descripción.
 * Actualiza la lista de socios extranjeros reflejando el nuevo elemento.
 */
private agregarSocioFisicoExtranjero(VALOR_FORMULARIO: any): void {
  const NUEVO_SOCIO_EXTRANJERO: DatosSociosTableExtranjeros = {
    taxID: VALOR_FORMULARIO.taxId || '',
    razonSocial: '',
...
private aplicarAccionistasRespuesta(apiResponse: any): void {
  ...
  DATOS.forEach((item: any) => {
    ...
    const RFC_EXIST = this.datosSocios.some(socio => socio.rfc === NUEVO_SOCIO.rfc);
    if (RFC_EXIST) {
      this.mostrarPopupError('El RFC que intenta agregar se encuentra previamente.');
      return;
    }
...
/**
 * Abre un modal de notificación para confirmar la eliminación de anexos.
 * Muestra un mensaje distinto según si existen socios seleccionados.
 * Configura una alerta tipo “danger” con opciones de aceptar o cancelar.
 */
  abrirTresModal(): void {
    this.alertaNotificacion = {
      ...
      titulo: '',
      mensaje:
        this.datosSocios.length > 0 ? '¿Está seguro de eliminar el registro de los anexos?' : 'Debe seleccionar accionistas a eliminar.',
      cerrar: true,
      tiempoDeEspera: 2000,
      txtBtnAceptar: 'Aceptar',
      txtBtnCancelar: this.datosSocios.length > 0 ? ' Cancelar' : '',
    };
  }

/**
 * Muestra un popup de error con un mensaje personalizado.
 * Configura una notificación de tipo alerta y la despliega por 2 segundos.
 * Ideal para informar al usuario sobre errores en acciones o formularios.
 */
mostrarPopupError(mensaje: string): void {
  this.alertaNotificacion = {
    tipoNotificacion: 'alert',
    categoria: 'danger',
    modo: 'action',
