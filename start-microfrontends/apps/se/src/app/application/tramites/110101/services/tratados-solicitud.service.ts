import { API_GET_CRITERIO_TRATADO_RESUMEN, API_GET_INSUMOS_EMPAQUES, API_POST_SOLICITUD_TRATADOS_CONFIGURACION, API_POST_TRATADO_CRITERIO, API_POST_VALIDACION_ACUMULACION } from "../server/api-router";
...
import { ValidacionAcumulacionResponse } from "../models/response/validacion-fraccion-acumulacion-response.model";
...
export class TratadosSolicitudService {
   ...
   * 
   * @param PAYLOAD - Datos de validación de acumulación de fracción arancelaria
   */
  postValidarAcumalacion(PAYLOAD: AcumulacionFraccionTratadosRequest): Observable<BaseResponse<ValidacionAcumulacionResponse>> {
    const ENDPOINT = `${this.host}` + API_POST_VALIDACION_ACUMULACION;
    return this.http.post<BaseResponse<ValidacionAcumulacionResponse>>(ENDPOINT, PAYLOAD);
  }
}