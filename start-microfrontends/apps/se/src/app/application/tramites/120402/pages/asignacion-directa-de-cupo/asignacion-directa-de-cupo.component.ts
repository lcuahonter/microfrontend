...
/**
 ...
 /**
   ...
   * @param {Tramite120402State} data - Estado completo de la solicitud a guardar
   * @returns {Observable<unknown>} Observable con la respuesta del servidor
   * @public
   */
  public guardar(data:Tramite120402State): Observable<unknown> {
    ...
    return this.service.obtenerGuardarSolicitud(PAYLOAD).pipe(
      ...
      map((response) => {
        if(esValidObject(response)) {
          ...
          this.tramite120402Store.setIdSolicitud(RESPONSE?.datos?.id_solicitud ?? 0);
