    ...
    this.consultaQuery.selectConsultaioState$
      ...
      .subscribe((seccionState) => {
        ...
        if (this.consultaState &&
          this.consultaState.procedureId === '80104' &&
          (this.consultaState.update)) {
          this.guardarDatosFormulario(this.consultaState?.id_solicitud);
          this.tramite80104Store.setIdSolicitud(
      ...
      .subscribe((resp) => {
        if (resp) {
          if (resp?.datos) {
            ...
            if (DATOS) {
              const MAPPED_DATA = this.Solocitud80104Service.reverseBuildSolicitud80104(DATOS as Record<string, unknown>) as unknown as Tramite80101State;
              this.Solocitud80104Service.actualizarEstadoFormulario(MAPPED_DATA);
            }
            this.esDatosRespuesta = true;
          }
        }
