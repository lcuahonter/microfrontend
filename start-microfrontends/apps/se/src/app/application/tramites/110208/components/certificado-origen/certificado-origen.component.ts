...
export class CertificadoOrigenComponent
   ...
   * @description Título mostrado en el componente.
   * Puede ser personalizado desde el componente padre mediante [title].
   */
  public title:string = 'Validación a posteriori del certificado de circulación de mercancías';
  /**
  ...
  abrirModificarModal(
    ...
    this.store.setmercanciaTabla([disponiblesDatos]);
  ...
  guardarClicado(evento: Mercancia[]): void {
    this.store.replaceMercanciaTabla(evento);
    if (evento.length > 0) {
      this.store.setFormMercancia({ ...evento[0] });
    }
