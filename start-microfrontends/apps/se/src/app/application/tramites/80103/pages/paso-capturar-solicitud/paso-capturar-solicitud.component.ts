 ...
 */
  ...
  guardar(data: Tramite80101State): Promise<JSONResponse> {
    // Acceder a los datos del FormControl almacenados en el store
    const formControlData = data.formControlData || {};
    // Acceder a los datos del FormControl del anexo almacenados en el store
    const anexoFormControlData = data.anexoFormControlData || {};
    // Acceder a los datos del FormControl de federatarios almacenados en el store
    const federatariosFormControlData = data.federatariosFormControlData || {};
    // Ahora puedes usar los datos del FormControl junto con los datos regulares
    ...
    function convertTestadoRecursive(obj: any): any {
      if (Array.isArray(obj)) {
        ...
        for (const KEY in obj) {
          if (KEY === 'testado') {
            NEW_OBJ[KEY] = obj[KEY] === '1';
          } else {
            NEW_OBJ[KEY] = convertTestadoRecursive(obj[KEY]);
          }
        }
        return NEW_OBJ;
      }
    ...
    const PLANT_ARRAY = convertTestadoRecursive(Array.isArray(PLANTAS) ? [...PLANTAS] : [PLANTAS]);
    ...
    const PAYLOAD = {
      "cveRolCapturista":"PersonaMoral",
       ...
       "idTipoTramite": 80103,
       "rfc": "AAL0409235E6",
       "cveUnidadAdministrativa": data.federatariosFormControlData?.['federatarios']?.estadoUno,
       "costoTotal": 10000.5,
