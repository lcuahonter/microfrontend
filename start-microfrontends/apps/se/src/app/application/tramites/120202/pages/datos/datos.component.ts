...
import { StoreValues } from '../../models/expedicion-certificados-asignacion.model';
...
export class DatosComponent implements OnInit, OnDestroy {
  ...
  */
  expedicionCertificadoAsignacionState!: ExpedicionCertificadosAsignacion120202State
  /**
   ...
   * @param tramite120202Store - Almacén para gestionar el estado del trámite 120202.
   * @param service - Servicio para gestionar los cupos.
   */
  constructor(
    private consultaQuery: ConsultaioQuery,
    private tramite120202Store: Tramite120202Store,
    private tramite120202Query: Tramite120202Query,
    private service: ExpedicionCertificadosAsignacionService
  ) {
  ...
  ngOnInit(): void {
    ...
    if (this.consultaState.update) {
      ...
      this.tramite120202Store.setIdSolicitud(this.consultaState.id_solicitud);
    } else {
  ...
  public storeMethodMap: Record<string, (campo: string, value: unknown) => void> = {
    asignacionOficioNumeroForm: (campo, v) =>
      this.tramite120202Store.setAsignacionOficioNumeroForm(campo, v),

    representacionFederalForm: (campo, v) =>
      this.tramite120202Store.setRepresentacionFederalForm(campo, v),

    controlMontosAsignacionForm: (campo, v) =>
      this.tramite120202Store.setControlMontosAsignacionForm(campo, v),

    asignacionDatosForm: (campo, v) =>
      this.tramite120202Store.setAsignacionDatosForm(campo, v),

    cupoDescripcionForm: (campo, v) =>
      this.tramite120202Store.setCupoDescripcionForm(campo, v),

    distribucionSaldoForm: (campo, v) =>
      this.tramite120202Store.setDistribucionSaldoForm(campo, v)
  };
  ...
  updateStoreValues(data: StoreValues): void {
    (Object.keys(data) as Array<keyof StoreValues>).forEach(key => {
      ...
      if (
        SETTER_NAME in this.tramite120202Store &&
        typeof (this.tramite120202Store as unknown as Record<string, unknown>)[SETTER_NAME] === 'function'
      ) {
        (this.tramite120202Store as unknown as Record<string, (value: StoreValues[typeof key]) => void>)[SETTER_NAME](data[key]);
      }
    });
  }
  ...
  guardarDatosFormulario(id_solicitud: string): void {
    this.service
      ...
      .subscribe((resp) => {
        if (resp) {
          this.esDatosRespuesta = true;
          this.service.reversePayload(resp['datos'] as unknown as Record<string, unknown>);
          this.tramite120202Store.update(state => ({
            ...state,
            ...{ ...(this.service.reversePayload(resp as unknown as Record<string, unknown>)) } as Record<string, unknown>
          }));
        }
      });
  }
