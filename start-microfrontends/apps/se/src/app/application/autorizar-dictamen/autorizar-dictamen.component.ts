...
import { CategoriaMensaje, ConsultaioQuery, ConsultaioState, ConsultaioStore, DesplazarseHaciaArribaService, FECHA_DE_INICIO, Notificacion, NotificacionesComponent, TabsConfigService, base64ToHex, encodeToISO88591Hex } from '@ng-mf/data-access-user';
...
import { AutorizarGenerarDictamen130Component } from '../shared/components/autorizar-generar-dictamen-130/autorizar-generar-dictamen.component';
...
@Component({
  ...
  imports: [CommonModule, ReviewersTabsComponent,
    ...
    AutorizarGenerarDictamen130Component,
 ...
 */
  ...
  /**
   * @property {string} selloOficio
   * @description Sello digital del oficio firmado.
   */
  selloOficio!: string;
 ...
 */
  ...
  constructor(private router: Router,
    ...
    */
    ...
    if (this.config.validacionesEspeciales) {
  ...
  */
  ...
  /**
   * Recibe los sellos digitales generados tras la firma electrónica.
   * @param sellos 
   */
  sellos(sellos: string[]): void {
    this.selloOficio = sellos[0];
  }
 ...
 */
  postOficioAutorizacion(): void {
    //All processes must send this payload; otherwise, it will need to be adjusted in the backend.
    const PAYLOAD: Firma = {
      cadena_original: encodeToISO88591Hex(this.firmaOficioCadena[0] || ''),
      cert_serial_number: this.datosFirmaReales.certSerialNumber,
      clave_usuario: this.datosFirmaReales.rfc,
      fecha_firma: AutorizarDictamenComponent.formatFecha(new Date()),
      clave_rol: 'Autorizador',
      sello: base64ToHex(this.selloOficio)
    }
    this.autorizarDictamenService.postOficioAutorizacion(this.tramite, Number(this.guardarDatos.id_solicitud), PAYLOAD)
      .subscribe({
        next: (resp) => {
 ...
 */
  postOficioRechazado(): void {
    ...
    this.autorizarDictamenService.postOficioRechazado(this.tramite, Number(this.guardarDatos.id_solicitud), PAYLOAD)
      .subscribe({
        next: (resp) => {
          if (resp.codigo === "00" && resp.datos) {
            this.datosTablaResolucion = [{
  ...
  */
  ...
  postTerminar(): void {
   if (
    this.config.validacionesEspeciales &&
    this.observacionForm.invalid
  ) {
