...
import { ConfiguracionColumna, InputFecha, InputFechaComponent, TablaDinamicaComponent, TituloComponent, ValidacionesFormularioService, formatDDMMYYYY, formatFechaDDMMYYYY, formatToDDMMYYYY } from "@libs/shared/data-access-user/src";
...
import { ModificarPartidasComponent } from "../modificar-partidas/modificar-partidas.component";
...
/**
 * @component
 * @name GenerarDictamen130Component
 ...
 */
@Component({
  selector: 'app-generar-dictamen-130',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class GenerarDictamen130Component implements OnInit, OnChanges, OnDestroy {
  /**
  ...
  */
  ...
  /** Indica si se deben modificar las partidas */
  @Input() public modificarPartidas = false;

  /** Indica si se está modificando las partidas */
  isModificarPartidasVisible: boolean = false;
  ...
  /**
   * @property {InputFecha} configuracionFechaFinVigenciaAutorizada
   * @description Configuración para el campo de fecha de fin de vigencia autorizada en el formulario de dictamen.
   * Incluye etiqueta, requisito y estado habilitado.
   */
  configuracionFechaFinVigenciaAutorizada: InputFecha = {
  ...
  /**
   * @property {InputFecha} configuracionFechaInicioVigenciaAutorizada
   * @description Configuración para el campo de fecha de inicio de vigencia autorizada en el formulario de dictamen.
   * Incluye etiqueta, requisito y estado habilitado.
   */
  configuracionFechaInicioVigenciaAutorizada: InputFecha = {
  ...
  ngOnInit(): void {
    this.dictamenForm = this.fb.group({
      ...
      mensajeDictamen: ['', [
        Validators.required,
        GenerarDictamen130Component.noSoloEspacios,
        Validators.maxLength(this.idTramite === 130109 ? 10000 : 2000)
      ]],
      ...
      fechaFinVigenciaAutorizada: [{ value: '', disabled: this.idTramite === 130109 ? false : true }],
    });
  ...
  private actualizarVisibilidadCamposFecha(valorCumplimiento: string): void {
    ...
    this.mostrarCamposFecha = this.habilitarFechasPorAceptacion;

    this.isModificarPartidasVisible = this.modificarPartidas;
   ...
   * @param event  Evento de cambio en el cumplimiento del dictamen.
   * 
   * Realiza la limpieza necesaria y completa el Subject de destrucción para evitar fugas de memoria.
   */
  onCumplimientoChange(event: Event): void {
    const INPUT_ELEMENT = event.target as HTMLInputElement;
    const VALOR_CUMPLIMIENTO = INPUT_ELEMENT.value;

    this.isModificarPartidasVisible = VALOR_CUMPLIMIENTO === 'SEDI.AC' && this.modificarPartidas;
    if (!this.isModificarPartidasVisible && VALOR_CUMPLIMIENTO === 'SEDI.RZ') {
      this.mostrarCamposFecha = false;
    } else {
      this.mostrarCamposFecha = true;
    }
  }

  onSentidoCumplimientoChange(clave: string): void {
    const VALOR_CUMPLIMIENTO = clave;
    this.isModificarPartidasVisible = VALOR_CUMPLIMIENTO === 'SEDI.AC' && this.modificarPartidas;
    if (!this.isModificarPartidasVisible && VALOR_CUMPLIMIENTO === 'SEDI.RZ') {
      this.mostrarCamposFecha = false;
    } else {
  ...
  /**
   * @method onFechaFinVigenciaAutorizadaChange
   * @description Maneja el cambio en la fecha de fin de vigencia autorizada y actualiza el formulario.
   * @param date Nueva fecha seleccionada.
   * @returns {void}
   */
  onFechaFinVigenciaAutorizadaChange(date: string): void {
    this.dictamenForm.patchValue({
      fechaFinVigenciaAutorizada: date,
    });
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['conformidad'] && this.dictamenForm) {
      this.dictamenForm.get('antecedentesReadonly')?.setValue(this.conformidad.texto_dictamen);
      if (!this.dataIniciarDictamen?.fecha_inicio_vigencia && !this.dataIniciarDictamen?.fecha_fin_vigencia) {
        this.dictamenForm.patchValue({
          fechaInicioVigenciaAutorizada: formatDDMMYYYY(this.conformidad.fecha_inicio),
          fechaFinVigenciaAutorizada: [130109, 130107, 130113, 130104].includes(this.idTramite) ? formatDDMMYYYY(this.conformidad.fecha_fin_vigencia) : this.conformidad.fecha_fin_vigencia,
        });
      }
    }
    if (changes['dataIniciarDictamen'] && changes['dataIniciarDictamen'].currentValue && this.dictamenForm) {
      ...
      if (this.dataIniciarDictamen.fecha_inicio_vigencia && this.dataIniciarDictamen.fecha_fin_vigencia) {
        this.dictamenForm.patchValue({
          fechaInicioVigenciaAutorizada: this.dataIniciarDictamen.fecha_inicio_vigencia ? formatToDDMMYYYY(this.dataIniciarDictamen.fecha_inicio_vigencia) : '',
          fechaFinVigenciaAutorizada: this.dataIniciarDictamen.fecha_fin_vigencia ? formatToDDMMYYYY(this.dataIniciarDictamen.fecha_fin_vigencia) : '',
        });
      }
      ...
      if (changes?.['modificarPartidas'] && changes?.['modificarPartidas'].currentValue) {
        this.modificarPartidas = changes?.['modificarPartidas'].currentValue;
      }
    }
    ...
    if (changes['dataIniciarDictamenAutorizar'] && changes['dataIniciarDictamenAutorizar'].currentValue) {
      ...
      this.dictamenForm.patchValue({
        ...
        fechaFinVigenciaAutorizada: [130109, 130107, 130113, 130104].includes(this.idTramite) ? formatFechaDDMMYYYY(this.conformidad.fecha_fin_vigencia) : this.dataIniciarDictamenAutorizar.fecha_fin_vigencia,
      });
