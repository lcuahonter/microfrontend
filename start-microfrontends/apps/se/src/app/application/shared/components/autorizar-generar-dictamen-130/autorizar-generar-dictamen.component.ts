...
/**
 * @component
 * @name AutorizarGenerarDictamen130Component
 ...
 */
@Component({
  selector: 'app-autorizar-generar-dictamen-130',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, TablaDinamicaComponent, TituloComponent, ModificarPartidasComponent],
  templateUrl: './autorizar-generar-dictamen.component.html',
  styleUrl: './autorizar-generar-dictamen.component.scss',
})
export class AutorizarGenerarDictamen130Component implements OnInit, OnChanges, OnDestroy {
  /**
  ...
  */
  ...
  ngOnInit(): void {
    this.dictamenForm = this.fb.group({
      ...
      mensajeDictamen: ['', [
        Validators.required,
        AutorizarGenerarDictamen130Component.noSoloEspacios,
  ...
  private actualizarVisibilidadCamposFecha(valorCumplimiento: string): void {
    ...
    this.isModificarPartidasVisible = this.modificarPartidas && ESDICTAMENACEPTADO;
  ...
  */
  ...
  /**
   *  @property {string} fechaInicioLabel
   *  @description Obtiene la etiqueta para el campo de fecha de inicio de vigencia autorizada
   *  dependiendo del valor del parámetro `parameter`.
   */
  get fechaInicioLabel(): string {
    return this.parameter === 'AutorizarDictamen'
      ? 'Fecha Inicial:'
      : '*Fecha de inicio de vigencia autorizada:';
  }

  /**
   * @property {string} fechaFinLabel
   * @description Obtiene la etiqueta para el campo de fecha de fin de vigencia autorizada
   * dependiendo del valor del parámetro `parameter`.
   */
  get fechaFinLabel(): string {
    return this.parameter === 'AutorizarDictamen'
      ? 'Fecha Final:'
