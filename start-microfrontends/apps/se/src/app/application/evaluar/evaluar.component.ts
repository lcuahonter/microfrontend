...
import { ReviewersTabs110101Component } from '../shared/components/reviewers-tabs-110101/reviewers-tabs-110101.component';
...
import { AlertComponent, CatalogoTipoDocumento, CategoriaMensaje, ConsultaioQuery, ConsultaioState, ConsultaioStore, DesplazarseHaciaArribaService, DocumentosEspecificosResponse, FECHA_DE_INICIO, MenuFuncionarioComponent, Notificacion, NotificacionesComponent, TabEvaluarTratadosResponse, TabsConfigService, base64ToHex, encodeToISO88591Hex, formatearFechaYyyyMmDd } from '@ng-mf/data-access-user';
...
import { CodigoRespuesta, SERIES8010 } from '../core/enum/se-core-enum';
...
import { GenerarDictamen130Component } from '../shared/components/generar-dictamen-130/generar-dictamen.component';
...
import { ValidacionDocumentosRequerimiento } from '../shared/models/validacion-documentos-requerimiento.model';
import { getFechaFinError } from '@libs/shared/data-access-user/src/tramites/constantes/mensajes-error-formularios';
...
export class EvaluarComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  /**
   * @property {number} indiceRequerimiento
   * @description Índice de la pestaña de requerimiento seleccionada.
   */
 ...
 */
  ...
  /**
   * @property {ValidacionDocumentosRequerimiento} documentosRequerimientoValidacion
   * @description Configuración de validación de documentos requerida para el trámite, obtenida del servicio TramiteConfigService.
   */
  documentosRequerimientoValidacion!: ValidacionDocumentosRequerimiento;
 ...
 */
  ...
  /**
   * @property {boolean} mostrarFechaFinError
   * @description Indica si se debe mostrar un error relacionado con la fecha de fin del certificado.
   */
  mostrarFechaFinError: boolean = false;

  /**
   * @property {string} ALERTA_FECHA_FIN_ERROR
   * @description Mensaje de error que se muestra cuando hay un problema con la fecha de fin del certificado.
   */
  ALERTA_FECHA_FIN_ERROR!: string;

  series8010 = SERIES8010;
  /**
 * @constructor
 ...
 */
  constructor(private router: Router,
    ...
    this.documentosRequerimientoValidacion = this.tramiteConfigService.getValidacionDocuemntosRequerimientos(this.tramite);
 ...
 */
      ...
      .subscribe({
        next: (response) => {
          if (response.codigo === CodigoRespuesta.EXITO) {
            ...
            const TRAMITE: number[] = [80101, 80102, 80103, 80104, 130113];
            if ((TRAMITE.includes(this.tramite)) && this.opcionesDisponibles.length > 1) {
              this.tabsOpcionEvaluacion = this.opcionesDisponibles.map((opcion, i) => ({
                id: opcion === 'Generar requerimiento de información' ? 2 : opcion === 'Solicitar opinión' ? 3 : 1,
                nombre: opcion
              }));
            } else {
              this.tabsOpcionEvaluacion = this.opcionesDisponibles.map((opcion, i) => ({
  ...
  guardarDictamen(datosDictamen?: DictamenForm): void {
    ...
    const ES_AUTORIZADO = datosDictamen.cumplimiento === 'SEDI.AC';

    const FECHA_INICIO = ES_AUTORIZADO
      ? this.conformidadDictamen.fecha_inicio ?? null
      : '';

    const FECHA_FIN = ES_AUTORIZADO
     ? this.conformidadDictamen.fecha_fin_vigencia ?? null
     : '';
    const TRAMITE = this.tramite?.toString();
    const EXCEPCIONES_130_MX = ['130118', '130120'];

    const FECHA_FIN_130 = formatearFechaYyyyMmDd(datosDictamen.fechaFinVigenciaAutorizada) ?? null;
    ...
    const PAYLOAD: GuardarDictamenRequest = {
      ...
      //It is not correct to perform a validation where the procedures begin with 130, since there are procedures that we have in Mexico that are 130118 and 130120 among others.
      fecha_fin_vigencia: this.tramite.toString().startsWith('130') && !EXCEPCIONES_130_MX.includes(TRAMITE) 
       ? FECHA_FIN_130 : FECHA_FIN ?? formatearFechaYyyyMmDd(datosDictamen.fechaFinVigenciaAutorizada) ?? null,
    ...
    this.guardarService.postGuadarDictamen(this.tramite, this.guardarDatos.folioTramite, PAYLOAD)
      .subscribe({
        next: (resp) => {
          if (resp.codigo === CodigoRespuesta.EXITO) {
            this.mostrarFechaFinError = false;
            window.scrollTo({ top: 0, behavior: 'smooth' });
            this.nuevaNotificacion = {
              tipoNotificacion: 'toastr',
            ...
            this.mostrarFechaFinError = true;
            this.ALERTA_FECHA_FIN_ERROR = getFechaFinError(resp.error ?? '')
          }
        },
        error: (err) => {
          window.scrollTo({ top: 0, behavior: 'smooth' });
  ...
  firmarMostrarDictamen(datosDictamen?: DictamenForm): void {
    ...
    const PAYLOAD: MostrarFirmarRequest = {
      ...
      fecha_inicio_vigencia: FECHA_INICIO,
      //It is not correct to perform a validation where the procedures begin with 130, since there are procedures that we have in Mexico that are 130118 and 130120 among others.
      fecha_fin_vigencia: this.tramite.toString().startsWith('130') && !EXCEPCIONES_130_MX.includes(TRAMITE) ? FECHA_FIN_130 : FECHA_FIN ?? formatearFechaYyyyMmDd(datosDictamen.fechaFinVigenciaAutorizada) ?? null,
      texto_dictamen: datosDictamen.antecedentesEditables ?? null,
      criterios_dictaminacion: this.conformidadDictamen.texto_dictamen ?? null,
      solicitante: {
  ...
  cancelar(): void {
    this.indice = 0;
    if(this.series8010.includes(this.tramite.toString())){
      this.indice=0;
      this.indiceDictamen=0;
    }
  ...
  guardarRequerimiento(): void {
    ...
    if(this.indiceRequerimiento === 2){
      const CONFIG = this.documentosRequerimientoValidacion;
  
      if (CONFIG.TablaDocumentosAdicionales) {
        if (this.listadoDocumentosEspecificos.length === 0) {
          this.validacionDocumentos = true;
          this.validarJustificacionRequerimiento();
          return;
        }
        this.validarJustificacionRequerimiento();
      } else if (CONFIG.TablaDocumentosRequeridos) {
      ...
      if (this.validacionJustificacion) {
        return;
      }
    } else if (this.indiceRequerimiento === 1) {
      this.validarJustificacionRequerimiento();
 ...
 */
  ...
  firmarRequerimiento(firma: string): void {
    ...
    const PAYLOAD: FirmarRequerimientoRequest = {
      ...
      cve_usuario: this.guardarDatos.current_user,
      requiere_autorizador: this.dataIniciarRequerimiento.requiere_autorizador
    };
      ...
      .pipe(
        ...
        tap((firmaResponse: BaseResponse<null>) => {
          if (firmaResponse.codigo !== '00') {
            ...
            localStorage.setItem('mensajeExito', 'REQUERIMIENTO');
            this.router.navigate(['bandeja-de-tareas-pendientes']);
          }
 ...
 */
 ...
 postOficioComision(): void {
    this.evaluarSolicitudService.postOficioComision(this.tramite, this.guardarDatos.id_solicitud, this.guardarDatos.folioTramite).subscribe({
      next: (resp) => {
  ...
  postVistaPreviaActa(): void {
    this.evaluarSolicitudService.postVistaPreviaActa(this.tramite, this.guardarDatos.id_solicitud, this.guardarDatos.folioTramite).subscribe({
      next: (resp) => {
        if (resp.codigo === CodigoRespuesta.EXITO && resp.datos?.contenido) {
          EvaluarComponent.manejarPdf(
            resp.datos.contenido,
