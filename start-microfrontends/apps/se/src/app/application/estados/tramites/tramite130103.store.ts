...
/**
 * 
 * Interfaz que define el estado del store `Tramite130103State`.
 * Este estado almacena toda la información relacionada con el trámite 130103.
 */
export interface Tramite130103State {
    /**
    ...
    /** Unidad de medida utilizada en el trámite. */
    unidadesMedida: string;
    /** Fracción arancelaria PROSEC asociada al trámite. */
    fraccionArancelariaProsec: string;
    /** Esquema de regla de la mercancía. */
    mercanciaEsquema: string;
    /** Criterio de dictamen asociado al trámite. */
    criterioDictamen: string;
    /** Descripción de la fracción PROSEC asociada al trámite. */
    descripcionFraccionProsec: string;
}

/**
 * 
 * Función que crea el estado inicial del store `Tramite130103Store`.
 * @returns {Tramite130103State} Estado inicial del store.
 */
export function createInitialState(): Tramite130103State {
    return {
        idSolicitud: 0,
        filaSeleccionada: [],
        mostrarTabla: false,
...
/**
 * 
 * Servicio que implementa el store `Tramite130103Store` para gestionar el estado
 * relacionado con el trámite 130103.
 *
 * @decorador @Injectable
 * @decorador @StoreConfig
 */
@Injectable({ providedIn: 'root' })
@StoreConfig({ name: 'tramite130103' })
export class Tramite130103Store extends Store<Tramite130103State> {
    /**
     * 
     * Constructor del store `Tramite130103Store`.
     ...
     */
    public actualizarEstado(valores: Partial<Tramite130103State>): void {
        this.update((state) => ({
            ...state,
            ...valores,
        }));
