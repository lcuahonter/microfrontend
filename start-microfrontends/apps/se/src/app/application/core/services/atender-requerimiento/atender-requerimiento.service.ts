import { API_GET_INICIAR_ATENDER_REQUERIMIENTO, API_POST_ACUSE_REQUERIMIENTO, API_POST_ATENCION_REQUERIMIENTO, API_POST_FIRMAR, API_POST_MOSTRAR_FIRMAR, IDSOLICITUD, NUMFOLIOTRAMITE, TRAMITE } from '../../../constantes/atender-requerimiento/api-constants';
...
import { AtencionRequerimiento } from '../../models/atender-requerimiento/request/documentos-especificos.model';
 ...
 */
  ...
  */
  ...
  /**
   * Guarda la atención del requerimiento.
   * @param tramite - Número de trámite asociado
   * @param numFolio - Número de folio del trámite
   * @param PAYLOAD - Datos de la atención del requerimiento
   * @returns Observable con la respuesta de la operación
   */
  postAtencionRequerimiento(tramite: number, numFolio: string, PAYLOAD: AtencionRequerimiento):
    Observable<BaseResponse<AtencionRequerimiento>> {
    const ENDPOINT = `${this.host}${API_POST_ATENCION_REQUERIMIENTO.replace(TRAMITE, tramite.toString()).replace(NUMFOLIOTRAMITE, numFolio)}`;
    return this.http.post<BaseResponse<AtencionRequerimiento>>(ENDPOINT, PAYLOAD);
  }
}
