...
import { API_GET_DESCARGAR_SOLICITUD, API_GET_EVALUAR_INICIAR, API_GET_EVALUAR_MOSTRAR, API_OBTENER_VISTA_PREVIA_ACTA, API_OBTENER_VISTA_PREVIA_REPORTE, API_POST_OPCIONES_EVALUACION_GUARDAR, API_TERMINAR_OPCIONES_EVALUACION } from '../../server/api-router';
 ...
 */
  ...
  /**
   * Guarda la solicitud de evaluación para un trámite específico.
   * @param tramite - Número del trámite.
   * @param folioTramite - Folio del trámite.
  ...
  /**
   * Agrega opciones de evaluación para un trámite específico.
   * @param tramite - Número del trámite.
   * @param folioTramite - Folio del trámite.
   * @param PAYLOAD - Datos a enviar en la solicitud.
   * @returns Observable con la respuesta del servidor.
   */
  ...
  /**
   * Envía un oficio de comisión para un trámite específico.
   * @param tramite - Número del trámite.
   * @param folioTramite - Folio del trámite.
   * @param PAYLOAD - Datos del oficio de comisión.
   * @returns Observable con la respuesta del servidor.
   */
  postOficioComision(tramite: number, solicitudId: string, folioTramite: string): Observable<BaseResponse<VistaPreliminarResponse>> {
    const ENDPOINT = `${this.host}${API_OBTENER_VISTA_PREVIA_REPORTE(tramite.toString(), solicitudId)}`;
    const BODY = { numeroFolioTramite: folioTramite };
    return this.http.post<BaseResponse<VistaPreliminarResponse>>(ENDPOINT, BODY);
  }

  /**
   * Obtiene la vista previa del acta circunstanciada para un trámite específico.
   * @param tramite - Número del trámite.
   * @param solicitudId - Identificador único de la solicitud.
   * @param folioTramite - Folio del trámite.
   * @returns Observable con la respuesta del servidor.
   */
  postVistaPreviaActa(tramite: number, solicitudId: string, folioTramite: string): Observable<BaseResponse<VistaPreliminarResponse>> {
    const ENDPOINT = `${this.host}${API_OBTENER_VISTA_PREVIA_ACTA(tramite.toString(), solicitudId)}`;
    const BODY = { numeroFolioTramite: folioTramite };
    return this.http.post<BaseResponse<VistaPreliminarResponse>>(ENDPOINT, BODY);
  }
}
