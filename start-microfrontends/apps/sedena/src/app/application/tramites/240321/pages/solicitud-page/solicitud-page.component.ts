import { AccionBoton, Catalogo, CategoriaMensaje, DatosPasos, ListaPasosWizard, Notificacion, PASOS, Usuario, WizardComponent } from '@ng-mf/data-access-user';
import { Component, EventEmitter, Output, ViewChild } from '@angular/core';
import { ID_PROCEDIMIENTO, TITULOMENSAJE } from '../../constants/solicitud-de-prorroga-importacion-material-explosivo.enum';
...
import { Tramite240321State, Tramite240321Store } from '../../estados/tramite240321Store.store';
...
export class SolicitudPageComponent {
   ...
   */
  public solicitudState!: Tramite240321State;
  ...
  public justificacion: JustificacionTramiteFormState = {
    justificacion: ''
  };
 ...
 * Inyecta los servicios necesarios para la gestión del trámite,
 * el acceso al estado y la obtención de los datos de la solicitud.
 *
 * @param tramiteQuery Servicio para consultar el estado del trámite.
 * @param tramiteStore Servicio para administrar y actualizar el estado del trámite.
 * @param datosSolicitudService Servicio para obtener la información de la solicitud.
 */
  constructor(private tramiteQuery: Tramite240321Query, private tramiteStore: Tramite240321Store,
 ...
 * Se suscribe al observable del índice de la pestaña seleccionada
 * para mantener sincronizado el subíndice del componente.
 * La suscripción se libera automáticamente al destruir el componente.
 */
  ngOnInit(): void {
 ...
 * Se utiliza para reflejar cambios de navegación
 * dentro del componente.
 *
 * @param i Índice de la subpestaña a establecer.
 */
  ...
  */
  private buildFinalPayload(
    ...
    return {
      ...
      justificacion: this.justificacion.justificacion,
  ...
  public async getValorIndice(e: AccionBoton): Promise<void> {
    if (e.accion === 'cont') {
      if (this.subIndice === 4) {
        ...
        this.justificacion = await firstValueFrom(this.tramiteQuery.getJustificacionTramite$)
        const formValid = await this.validarFormulario(DatosDelTramite);
        if (!formValid) return;
        ...
        const pagoValid = await this.validarPagoDerechos(PagoDerechos);
        if (!pagoValid) return;
 ...
 * @returns Promise<boolean> - Resultado de la validación.
 */
  validarPagoDerechos(data: any): Promise<boolean> {
    const banco = this.bancos.find(
  ...
  private mapDestinatarioToApiFormat(item: any) {
  ...
  private mapProveedorToApiFormat(item: any) {
    const telefonoCompleto = item.telefono?.trim() ?? null;
    ...
    return {
      ...
      primer_apellido: item.primerApellido ?? null,
      segundo_apellido: item.segundoApellido ?? null,
      denominacion_razon_social: item.denominacion_razon_social ?? null,
      codigo_postal: item.codigoPostal ?? null,
      numero_exterior: item.numeroExterior ?? null,
      numero_interior: item.numeroInterior ?? null,
      correo_electronico: item.correoElectronico ?? null
