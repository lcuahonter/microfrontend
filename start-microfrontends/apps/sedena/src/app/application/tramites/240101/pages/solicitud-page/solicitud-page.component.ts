...
@Component({
  ...
  imports: [BtnContinuarComponent, CommonModule, WizardComponent, PasoUnoComponent, PasoDosComponent, PasoTresComponent, NotificacionesComponent],
})
 ...
 */
   ...
   */
  public solicitudState!: Tramite240101State;
 ...
 * incluyendo la comunicación con el backend, el acceso al estado
 * del trámite y la consulta de datos almacenados.
 *
 * @param datosSolicitudService Servicio encargado de las operaciones relacionadas con la solicitud.
 * @param tramiteStore Store que administra el estado del trámite.
 * @param tramiteQuery Query para obtener información reactiva del estado del trámite.
 */
  constructor(
   ...
   *
   * Se suscribe al observable `getTabSeleccionado$` para escuchar los cambios
   * del tab seleccionado dentro del trámite y actualizar el índice local
   * `subIndice`.  
   * La suscripción se cancela automáticamente al destruir el componente
   * mediante `destroyNotifier$` para evitar fugas de memoria.
   */
  ngOnInit(): void {
